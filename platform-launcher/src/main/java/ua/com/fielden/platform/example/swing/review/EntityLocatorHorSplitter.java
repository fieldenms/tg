...
public class EntityLocatorHorSplitter extends EntityReview<Wheelset, IWheelsetDao, EntityQueryCriteria<Wheelset, IWheelsetDao>> {

    private static final long serialVersionUID = 2945111517158537350L;

    public EntityLocatorHorSplitter(final EntityReviewModel<Wheelset, IWheelsetDao, EntityQueryCriteria<Wheelset, IWheelsetDao>> model) {
	this(model, false);
    }

    public EntityLocatorHorSplitter(final EntityReviewModel<Wheelset, IWheelsetDao, EntityQueryCriteria<Wheelset, IWheelsetDao>> model, final boolean showRecords) {
	...
	if (getCriteriaPanel() == null) {
	    ...
	    splitManager.split(SplitEnum.NORTH, "NORTH");
	    splitManager.setComponent("NORTH", getCriteriaPanel(), JSplitPane.TOP);
	    ...
	    splitManager.setComponent("NORTH", gridViewPanel, JSplitPane.BOTTOM);
