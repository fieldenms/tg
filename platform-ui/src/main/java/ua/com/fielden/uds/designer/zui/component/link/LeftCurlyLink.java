...
public class LeftCurlyLink extends AbstractLink {
    ...
    public LeftCurlyLink(final PNode startNode, final PNode endNode) {
	super();
	setStroke(new DefaultStroke(1));
	compose(startNode, endNode);
    }

    public LeftCurlyLink(final PNode startNode, final PNode endNode, final LinkArrowLocation linkArrowLocation) {
	super();
	setStroke(new DefaultStroke(1));
	...
	Point2D.Double endNodeJoint = new Point2D.Double(endNodeBoundCentre.getX() + endNodeBound.getWidth() / 2., endNodeBoundCentre.getY());
	// check where endNodeJoint should actually be...
	if ((endNodeJoint.getX() - arcDepth) > startNodeJoint.getX()) {
	    endNodeJoint = new Point2D.Double(endNodeBoundCentre.getX() - endNodeBound.getWidth() / 2., endNodeBoundCentre.getY());
	}
	...
	line = new PPath();
	line.moveTo((float) incomingArc.points.get(3).getX(), (float) incomingArc.points.get(3).getY());
	...
	final double endArcX = startNodeJoint.getX() + arcDepth;
	...
	final Point2D.Double endArcMiddle2 = new Point2D.Double(startArc.getX() + arcDepth, startArc.getY()); // second approximation point starting from startArc
	...
	final Point2D.Double startArc = new Point2D.Double(startNodeJoint.getX() + arcDepth, middle.getY());

	double endArcX = startNodeJoint.getX();
	double delta = arcDepth;
	if (endArcX < endNodeJoint.getX()) {
	    endArcX = endNodeJoint.getX();
	    delta = startArc.getX() - endArcX;
	    if (Math.abs(delta) > arcDepth) {
		delta = -arcDepth;
		endArcX = startArc.getX() + arcDepth;
	    }
	}

	final Point2D.Double endArc = new Point2D.Double(endArcX, endNodeJoint.getY());

	// TODO at the moment endArcMiddle1 == endArcMiddle2
	final Point2D.Double endArcMiddle1 = new Point2D.Double(endArc.getX() + delta, endArc.getY());
	final Point2D.Double endArcMiddle2 = new Point2D.Double(endArc.getX() + delta, endArc.getY());
