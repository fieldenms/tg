...
public class CurlyLink extends AbstractLink {
    ...
    public CurlyLink(final PNode startNode, final PNode endNode) {
	super();
	setStroke(new DefaultStroke(1));
	compose(startNode, endNode);
    }

    public CurlyLink(final PNode startNode, final PNode endNode, final LinkArrowLocation linkArrowLocation) {
	super();
	setStroke(new DefaultStroke(1));
	...
	line = new PPath();
	line.moveTo((float) outgoingArc.points.get(3).getX(), (float) outgoingArc.points.get(3).getY());
	line.lineTo((float) incomingArc.points.get(3).getX(), (float) incomingArc.points.get(3).getY());
	...
	final double dist = Math.abs(startNodeJoint.getY() - middle.getY()) / 2.;

	final double endArcX = startNodeJoint.getX() - dist * perc < middle.getX() ? middle.getX() : startNodeJoint.getX() - dist * perc;
	...
	final double dist = Math.abs(endNodeJoint.getY() - middle.getY()) / 2.;
	final double endArcX = endNodeJoint.getX() + dist * perc > middle.getX() ? middle.getX() : endNodeJoint.getX() + dist * perc;
