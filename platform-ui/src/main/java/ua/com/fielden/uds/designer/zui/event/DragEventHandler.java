...
/**
 * This is an event handler for handling D&D activities (and deletion node tree by double click).
 * 
 * @author 01es
 * 
 */
public class DragEventHandler extends PDragEventHandler {
    ...
    /**
     * This methods prepares a node for being dragged (e.g. makes it transparent), and creates a cloned node, which is used as a placeholder to visualise where the node being
     * dragged was originally located. Also when double clicked - it deletes pickedBlock.
     */
    @Override
    protected void startDrag(final PInputEvent event) {
        ...
        final Point2D deltaMouse = new Point2D.Double(event.getPositionRelativeTo(draggedNode.getParent()).getX() - coords.getX(), event.getPositionRelativeTo(draggedNode.getParent()).getY()
                - coords.getY());
    ...
    /**
     * This a static inner class, which caters for a sequential execution of all animations and drop related actions.
     * 
