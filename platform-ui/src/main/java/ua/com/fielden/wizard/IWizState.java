...
/**
 * A class that represents a single state that is associated with a wizard page. The state serves as a model for associated with it view, which can be obtained by using method
 * <code>view()</code>.
 * 
 * @author TG Team
 * 
...
public interface IWizState<T extends AbstractEntity<?>> {
    ...
    /**
     * Returns a state that led to this state. Could be <code>null</code> if the current state represents the start.
     * 
     * @return
     */
