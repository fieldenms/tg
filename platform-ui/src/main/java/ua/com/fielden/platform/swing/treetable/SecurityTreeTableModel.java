...
public class SecurityTreeTableModel extends DynamicTreeTableModel {
     ...
     */
    public SecurityTreeTableModel(final Set<SecurityTokenNode> securityTokens, final Map<Class<? extends ISecurityToken>, List<UserRole>> mapOfAssociation, final List<UserRole> availableUserRoles) {
     ...
     */
    public void loadData(final Set<SecurityTokenNode> securityTokens, final Map<Class<? extends ISecurityToken>, List<UserRole>> mapOfAssociation, final List<UserRole> availableUserRoles) {

	this.availableUserRoles.clear();
	this.securityTokens.clear();
	this.availableUserRoles.addAll(availableUserRoles);

	final SecurityTreeTableNode root = new SecurityTreeTableNode(new ArrayList<Boolean>(), ISecurityToken.class, new ArrayList<UserRole>());
    ...
    private void buildTreeModel(final AbstractMutableTreeTableNode root, final Set<SecurityTokenNode> tokens, final Map<Class<? extends ISecurityToken>, List<UserRole>> mapOfAssociation) {
	for (final SecurityTokenNode tokenNode : tokens) {
	    final List<UserRole> userRoles = mapOfAssociation.get(tokenNode.getToken());
     ...
     * @return
     */
    public Map<Class<? extends ISecurityToken>, List<UserRole>> getModelData() {
     ...
     * @param securityTokens
     */
    private Map<Class<? extends ISecurityToken>, List<UserRole>> saveTreeModel(final SecurityTreeTableNode treeTableNode) {
	...
	final List<UserRole> selectedUserRoles = new ArrayList<UserRole>();
