...
public abstract class AbstractQueryDashboardItem<RESULT extends IDashboardItemResult, UI extends JFXPanel & IDashboardItemUi<RESULT> & IUmViewOwner> extends AbstractDashboardItem<RESULT, UI> {
    ...
    public static class AggregatedQueryBody implements IQueryBody<EntityAggregates> {
        ...
        public List<EntityAggregates> run(final List<QueryProperty> customParams) {
            return neededCount == null ? controller.getAllEntities(aggregatedQueryComposer.composeQuery(customParams))
                    : controller.getFirstEntities(aggregatedQueryComposer.composeQuery(customParams), neededCount);
        }
    ...
    public static class QueryBody<M extends AbstractEntity<?>> implements IQueryBody<M> {
        ...
        public List<M> run(final List<QueryProperty> customParams) {
            return neededCount == null ? controller.getAllEntities(queryComposer.composeQuery(customParams))
                    : controller.getFirstEntities(queryComposer.composeQuery(customParams), neededCount);
        }
    ...
    public static class GeneratedQueryBody<M extends AbstractEntity<?>> implements IQueryBody<M> {
        ...
        public List<M> run(final List<QueryProperty> customParams) {
            ...
            return neededCount == null ? controller.getAllEntities(queryComposer.composeQuery(customParams), managedTypeArrays)
                    : controller.getFirstEntities(queryComposer.composeQuery(customParams), neededCount, managedTypeArrays);
        }
