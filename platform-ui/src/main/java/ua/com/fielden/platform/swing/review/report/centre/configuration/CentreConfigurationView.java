...
public abstract class CentreConfigurationView<T extends AbstractEntity<?>, C extends AbstractEntityCentre<T, ICentreDomainTreeManagerAndEnhancer>> extends AbstractConfigurationView<C, EntityCentreWizard<T, C>> {
    ...
    public ICloseGuard canClose() {
        ...
        if (isChanged) {
            switch (JOptionPane.showConfirmDialog(null, "Would you like to save " + (isOwner ? "" : "as ") + "changes" + (!StringUtils.isEmpty(title) ? " for the " + title : "")
                    + " before closing?", "Save report", JOptionPane.YES_NO_CANCEL_OPTION, JOptionPane.QUESTION_MESSAGE)) {
    ...
    public void selectAnalysis(final String analysisToSelect) {
        if (ReportMode.REPORT == getModel().getMode() && getPreviousView() != null && getPreviousView().isVisible() && getPreviousView().isLoaded()) {
