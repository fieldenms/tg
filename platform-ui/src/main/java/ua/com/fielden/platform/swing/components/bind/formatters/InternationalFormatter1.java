...
/**
 * <code>InternationalFormatter</code> extends <code>DefaultFormatter</code>, using an instance of <code>java.text.Format</code> to handle the conversion to a String, and the
 * conversion from a String.
 * <p>
 * If <code>getAllowsInvalid()</code> is false, this will ask the <code>Format</code> to format the current text on every edit.
 * <p>
 * You can specify a minimum and maximum value by way of the <code>setMinimum</code> and <code>setMaximum</code> methods. In order for this to work the values returned from
 * <code>stringToValue</code> must be comparable to the min/max values by way of the <code>Comparable</code> interface.
 * <p>
 * Be careful how you configure the <code>Format</code> and the <code>InternationalFormatter</code>, as it is possible to create a situation where certain values can not be input.
 * Consider the date format 'M/d/yy', an <code>InternationalFormatter</code> that is always valid (<code>setAllowsInvalid(false)</code>), is in overwrite mode (
 * <code>setOverwriteMode(true)</code>) and the date 7/1/99. In this case the user will not be able to enter a two digit month or day of month. To avoid this, the format should be
 * 'MM/dd/yy'.
 * <p>
 * If <code>InternationalFormatter</code> is configured to only allow valid values (<code>setAllowsInvalid(false)</code>), every valid edit will result in the text of the
 * <code>JFormattedTextField</code> being completely reset from the <code>Format</code>. The cursor position will also be adjusted as literal characters are added/removed from the
 * resulting String.
 * <p>
 * <code>InternationalFormatter</code>'s behavior of <code>stringToValue</code> is slightly different than that of <code>DefaultTextFormatter</code>, it does the following:
 * <ol>
 * <li><code>parseObject</code> is invoked on the <code>Format</code> specified by <code>setFormat</code>
 * <li>If a Class has been set for the values (<code>setValueClass</code>), supers implementation is invoked to convert the value returned from <code>parseObject</code> to the
 * appropriate class.
 * <li>If a <code>ParseException</code> has not been thrown, and the value is outside the min/max a <code>ParseException</code> is thrown.
 * <li>The value is returned.
 * </ol>
 * <code>InternationalFormatter</code> implements <code>stringToValue</code> in this manner so that you can specify an alternate Class than <code>Format</code> may return.
 * <p>
 ...
 * @since 1.4
 */
public class InternationalFormatter1 extends DefaultFormatter1 {
    /**
    ...
    /**
     * <code>InternationalFormatter</code>'s behavior is dicatated by a <code>AttributedCharacterIterator</code> that is obtained from the <code>Format</code>. On every edit,
     * assuming allows invalid is false, the <code>Format</code> instance is invoked with <code>formatToCharacterIterator</code>. A <code>BitSet</code> is also kept upto date with
     * the non-literal characters, that is for every index in the <code>AttributedCharacterIterator</code> an entry in the bit set is updated based on the return value from
     * <code>isLiteral(Map)</code>. <code>isLiteral(int)</code> then uses this cached information.
     * <p>
     * If allowsInvalid is false, every edit results in resetting the complete text of the JTextComponent.
     * <p>
     * InternationalFormatterFilter can also provide two actions suitable for incrementing and decrementing. To enable this a subclass must override
     * <code>getSupportsIncrement</code> to return true, and override <code>adjustValue</code> to handle the changing of the value. If you want to support changing the value
     * outside of the valid FieldPositions, you will need to override <code>canIncrement</code>.
     */
    /**
    ...
    /**
     * Sets the minimum permissible value. If the <code>valueClass</code> has not been specified, and <code>minimum</code> is non null, the <code>valueClass</code> will be set to
     * that of the class of <code>minimum</code>.
     * 
     * @param minimum
    ...
    /**
     * Sets the maximum permissible value. If the <code>valueClass</code> has not been specified, and <code>max</code> is non null, the <code>valueClass</code> will be set to that
     * of the class of <code>max</code>.
     * 
     * @param max
    ...
    /**
     * Installs the <code>DefaultFormatter</code> onto a particular <code>JFormattedTextField</code>. This will invoke <code>valueToString</code> to convert the current value from
     * the <code>JFormattedTextField</code> to a String. This will then install the <code>Action</code>s from <code>getActions</code>, the <code>DocumentFilter</code> returned from
     * <code>getDocumentFilter</code> and the <code>NavigationFilter</code> returned from <code>getNavigationFilter</code> onto the <code>JFormattedTextField</code>.
     * <p>
     * Subclasses will typically only need to override this if they wish to install additional listeners on the <code>JFormattedTextField</code>.
     * <p>
     * If there is a <code>ParseException</code> in converting the current value to a String, this will set the text to an empty String, and mark the
    ...
    /**
     * Returns the <code>Format.Field</code> constants associated with the text at <code>offset</code>. If <code>offset</code> is not a valid location into the current text, this
    ...
    /**
     * Updates the AttributedCharacterIterator by invoking <code>formatToCharacterIterator</code> on the <code>Format</code>. If this is successful,
    ...
    /**
     * Returns true if the character at offset is navigatable too. This is implemented in terms of <code>isLiteral</code>, subclasses may wish to provide different behavior.
     */
    @Override
     ...
     * <ol>
     * <li>If the character is a literal, add it to the string to replace with. If there is text to insert and it doesn't match the literal, then insert the literal in the the
     * middle of the insert text. This allows you to either paste in literals or not and get the same behavior.
     * <li>If there is no text to insert, replace it with ' '.
     * </ol>
     * If not in overwrite mode, and there is text to insert it is inserted at the next non literal index going forward. If there is only text to remove, it is removed from the
     * next non literal index going backward.
     */
    @Override
    ...
    /**
     * When in !allowsInvalid mode the text is reset on every edit, thus supers implementation will position the cursor at the wrong position. As such, this invokes supers
     * implementation and then invokes <code>repositionCursor</code> to correctly reset the cursor.
     */
    @Override
    ...
    /**
     * Repositions the cursor. <code>startLiteralCount</code> gives the number of literals to the start of the deleted range, end gives the ending location to adjust from,
    ...
    /**
     * Subclasses supporting incrementing must override this to handle the actual incrementing. <code>value</code> is the current value, <code>attributes</code> gives the field the
    ...
    /**
     * Returns false, indicating InternationalFormatter does not allow incrementing of the value. Subclasses that wish to support incrementing/decrementing the value should
