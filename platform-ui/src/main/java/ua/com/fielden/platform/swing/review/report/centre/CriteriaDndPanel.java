...
public class CriteriaDndPanel extends StubCriteriaPanel {
    ...
    private void swapProperties(final LinkedComponentCopy compCopy1, final LinkedComponentCopy compCopy2) {
        final IAddToCriteriaTickManager firstTick = eqc.getCentreDomainTreeMangerAndEnhancer().getFirstTick();
        final String propName1 = compCopy1.getPropEditor() instanceof RangePropertyEditor ? ((RangePropertyEditor) compCopy1.getPropEditor()).getFromEditor().getPropertyName()
                : compCopy1.getPropEditor().getPropertyName();
        final String propName2 = compCopy2.getPropEditor() instanceof RangePropertyEditor ? ((RangePropertyEditor) compCopy2.getPropEditor()).getFromEditor().getPropertyName()
                : compCopy2.getPropEditor().getPropertyName();
    ...
    private static int maxLabelWidth(final Collection<IPropertyEditor> editors) {
        ...
        for (final IPropertyEditor editor : editors) {
            final boolean isNotSecondLabel = editor instanceof RangePropertyEditor ? true
                    : !CriteriaReflector.isSecondParam(editor.getEntity().getClass(), editor.getPropertyName());
