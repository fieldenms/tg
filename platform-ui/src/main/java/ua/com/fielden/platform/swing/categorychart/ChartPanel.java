...
public class ChartPanel extends JPanel implements ChartChangeListener, ChartProgressListener, ActionListener, MouseListener, MouseMotionListener, OverlayChangeListener, Printable, Serializable {
    ...
    /**
     * Default setting for buffer usage. The default has been changed to <code>true</code> from version 1.0.13 onwards, because of a severe performance problem with drawing the
     * zoom rectangle using XOR (which now happens only when the buffer is NOT used).
     */
    ...
    /**
     * The zoom rectangle starting point (selected by the user with a mouse click). This is a point on the screen, not the chart (which may have been scaled up or down to fit the
     * panel).
     */
    ...
    /**
     * Constructs a panel containing a chart. The <code>useBuffer</code> flag controls whether or not an offscreen <code>BufferedImage</code> is maintained for the chart. If the
     * buffer is used, more memory is consumed, but panel repaints will be a lot quicker in cases where the chart itself hasn't changed (for example, when another frame is moved to
     * reveal the panel). WARNING: If you set the <code>useBuffer</code> flag to false, note that the mouse zooming rectangle will (in that case) be drawn using XOR, and there is a
     * SEVERE performance problem with that on JRE6 on Windows.
     * 
     * @param chart
     *            the chart.
     * @param useBuffer
     *            a flag controlling whether or not an off-screen buffer is used (read the warning above before setting this to <code>false</code>).
     */
     ...
     * @param useBuffer
     *            a flag that indicates whether to use the off-screen buffer to improve performance (at the expense of memory).
     * @param properties
     *            a flag indicating whether or not the chart property editor should be available via the popup menu.
     * @param save
     *            a flag indicating whether or not save options should be available via the popup menu.
     ...
     * <P>
     * At the time the chart is drawn on the panel, if the available width is less than this amount, the chart will be drawn using the minimum width then scaled down to fit the
     * available space.
     * 
     * @param width
     *            The width.
     ...
     * <P>
     * At the time the chart is drawn on the panel, if the available width is greater than this amount, the chart will be drawn using the maximum width then scaled up to fit the
     * available space.
     * 
     * @param width
     *            The width.
     ...
     * <P>
     * At the time the chart is drawn on the panel, if the available height is less than this amount, the chart will be drawn using the minimum height then scaled down to fit the
     * available space.
     * 
     * @param height
     *            The height.
     ...
     * <P>
     * At the time the chart is drawn on the panel, if the available height is greater than this amount, the chart will be drawn using the maximum height then scaled up to fit the
     * available space.
     * 
     * @param height
     *            The height.
    ...
    /**
     * The mouse wheel handler. This will be an instance of MouseWheelHandler but we can't reference that class directly because it depends on JRE 1.4 and we still want to support
     * JRE 1.3.1.
     */
    ...
    /**
     * Enables or disables mouse wheel support for the panel. Note that this method does nothing when running JFreeChart on JRE 1.3.1, because that older version of the Java
     * runtime does not support mouse wheel events.
     * 
     * @param flag
     *            a boolean.
     * 
    ...
    /**
     * Paints the component by drawing the chart to fill the entire component, but allowing for the insets (which will be non-zero if a border has been set for this component). To
     * increase performance (at the expense of memory), an off-screen buffer image can be used.
     * 
     * @param g
    ...
    public void paintComponent(final Graphics g) {
        ...
        if (this.useBuffer) {
            ...
            if (this.refreshBuffer) {
                ...
                final Rectangle2D bufferArea = new Rectangle2D.Double(0, 0, this.chartBufferWidth, this.chartBufferHeight);
        ...
        final Iterator iterator = this.overlays.iterator();
        ...
        if (this.panLast != null) {
            ...
            if (zoomTrigger1 || zoomTrigger2) {
                if ((hZoom && (e.getX() < this.zoomPoint.getX())) || (vZoom && (e.getY() < this.zoomPoint.getY()))) {
                    ...
                    double x, y, w, h;
                    final Rectangle2D screenDataArea = getScreenDataArea((int) this.zoomPoint.getX(), (int) this.zoomPoint.getY());
    ...
    /**
     * Decreases the length of the domain axis, centered about the given coordinate on the screen. The length of the domain axis is reduced by the value of
    ...
    /**
     * Increases the length of the domain axis, centered about the given coordinate on the screen. The length of the domain axis is increased by the value of
    ...
    /**
     * Draws zoom rectangle (if present). The drawing is performed in XOR mode, therefore when this method is called twice in a row, the second call will completely restore the
     * state of the canvas.
     * 
     * @param g2
     *            the graphics device.
    ...
    public void doEditChartProperties() {
        ...
        final int result = JOptionPane.showConfirmDialog(this, editor, localizationResources.getString("Chart_Properties"), JOptionPane.OK_CANCEL_OPTION, JOptionPane.PLAIN_MESSAGE);
    ...
    public void doCopy() {
        ...
        final ChartTransferable selection = new ChartTransferable(this.chart, getWidth(), getHeight());
    ...
    public void doSaveAs() throws IOException {
        ...
        final ExtensionFileFilter filter = new ExtensionFileFilter(localizationResources.getString("PNG_Image_Files"), ".png");
    ...
    protected JPopupMenu createPopupMenu(final boolean properties, final boolean copy, final boolean save, final boolean print, final boolean zoom) {
        ...
        if (properties) {
            final JMenuItem propertiesItem = new JMenuItem(localizationResources.getString("Properties..."));
        ...
        if (save) {
            ...
            final JMenuItem saveItem = new JMenuItem(localizationResources.getString("Save_as..."));
        ...
        if (print) {
            ...
            final JMenuItem printItem = new JMenuItem(localizationResources.getString("Print..."));
        ...
        if (zoom) {
            ...
            final JMenu zoomInMenu = new JMenu(localizationResources.getString("Zoom_In"));
            ...
            final JMenu zoomOutMenu = new JMenu(localizationResources.getString("Zoom_Out"));
            ...
            final JMenu autoRangeMenu = new JMenu(localizationResources.getString("Auto_Range"));
