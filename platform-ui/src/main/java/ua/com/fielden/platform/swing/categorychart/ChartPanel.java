...
MouseMotionListener, OverlayChangeListener, Printable, Serializable {
     ...
     *                  enabled for the chart.
     */
    public ChartPanel(final JFreeChart chart, final int width, final int height,
	...
	final Rectangle2D chartArea = new Rectangle2D.Double(0.0, 0.0, drawWidth,
		drawHeight);
	...
	// are we using the chart buffer?
		if (this.useBuffer) {
		    ...
		    // do we need to resize the buffer?
				...
				final Rectangle2D bufferArea = new Rectangle2D.Double(
					0, 0, this.chartBufferWidth, this.chartBufferHeight);

				final Graphics2D bufferG2 = (Graphics2D)
				this.chartBuffer.getGraphics();
				final Rectangle r = new Rectangle(0, 0, this.chartBufferWidth,
					this.chartBufferHeight);
				bufferG2.setPaint(getBackground());
				bufferG2.fill(r);
				if (scale) {
				    final AffineTransform saved = bufferG2.getTransform();
				    final AffineTransform st = AffineTransform.getScaleInstance(
	...
	if ((mods & this.panMask) == this.panMask) {
		...
		if (pannable.isDomainPannable() || pannable.isRangePannable()) {
		    final Rectangle2D screenDataArea = getScreenDataArea(e.getX(),
			    e.getY());
	...
	if (this.panLast != null) {
	    ...
	    final boolean zoomTrigger1 = hZoom && Math.abs(e.getX()
		    - this.zoomPoint.getX()) >= this.zoomTriggerDistance;
		    final boolean zoomTrigger2 = vZoom && Math.abs(e.getY()
	...
	// new entity code...
	final Object[] listeners = this.chartMouseListeners.getListeners(
	...
	final ChartMouseEvent chartEvent = new ChartMouseEvent(getChart(), event,
		entity);
	...
	if ((selection.getHeight() > 0) && (selection.getWidth() > 0)) {
	    ...
	    final double hLower = (selection.getMinX() - scaledDataArea.getMinX())
	    / scaledDataArea.getWidth();
	    final double hUpper = (selection.getMaxX() - scaledDataArea.getMinX())
	    / scaledDataArea.getWidth();
	    final double vLower = (scaledDataArea.getMaxY() - selection.getMaxY())
	    / scaledDataArea.getHeight();
	    final double vUpper = (scaledDataArea.getMaxY() - selection.getMinY())
	    / scaledDataArea.getHeight();
	...
	if (plot instanceof Zoomable) {
	    ...
	    final Point2D zp = (this.zoomPoint != null
	...
	final ChartTransferable selection = new ChartTransferable(this.chart,
		getWidth(), getHeight());
     ...
     * @return The popup menu.
     */
    protected JPopupMenu createPopupMenu(final boolean properties, final boolean save,
	...
	if (properties) {
	    final JMenuItem propertiesItem = new JMenuItem(
	...
	if (print) {
	    ...
	    final JMenuItem printItem = new JMenuItem(
	...
	if (zoom) {
	    ...
	    final JMenu zoomInMenu = new JMenu(
	    ...
	    final JMenu zoomOutMenu = new JMenu(
	    ...
	    final JMenu autoRangeMenu = new JMenu(
