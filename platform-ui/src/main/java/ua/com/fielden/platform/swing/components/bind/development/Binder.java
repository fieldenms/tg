        ...
        if (boundedMetaProperty != null) {
            return !boundedMetaProperty.isValid() ? boundedMetaProperty.getFirstFailure().getMessage()
                    : (boundedMetaProperty.hasWarnings() ? boundedMetaProperty.getFirstWarning().getMessage()
                            : (!byValue ? originalToolTipText
                                    : createToolTipByValue(boundedMetaProperty.getType(), boundedMetaProperty.getPropertyAnnotationType(), subjectValue, originalToolTipText, stringBinding, isMulti)));
        } else {
            final AbstractEntity eeeentity = (AbstractEntity) Rebinder.getActualEntity(entity);
            final Class<AbstractEntity> actualEntityType = (eeeentity instanceof EntityQueryCriteria ? ((Class<AbstractEntity>) ((EntityQueryCriteria) eeeentity).getEntityClass())
                    : eeeentity.getType());
            ...
            SwingUtilitiesEx.invokeLater(new Runnable() {
                ...
                public void run() {
                    boundedValidationlayer.setResult(!boundedMetaProperty.isValid() ? boundedMetaProperty.getFirstFailure()
                            : (boundedMetaProperty.hasWarnings() ? boundedMetaProperty.getFirstWarning() : new Result(boundedMetaProperty.getEntity(), "everything is cool")));
