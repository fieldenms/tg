...
public class EntitiesTreeModel2<DTM extends IDomainTreeManager> extends MultipleCheckboxTreeModel2 {
     ...
     * @param root
     * @param property
     * @param withDummyNaming
     *            -- indicates whether a property can contain "dummy" or "common" properties
     * @return
     */
    ...
    public static Pair<String, String> extractTitleAndDesc(final Class<?> root, final Class<?> managedType, final String property) {
        ...
        if (EntitiesTreeModel2.ROOT_PROPERTY.equals(property)) { // root node
            ...
            final Pair<String, String> tad = "".equals(property) ? TitlesDescsGetter.getEntityTitleAndDesc(root)
                    : TitlesDescsGetter.getTitleAndDesc(AbstractDomainTree.reflectionProperty(property), managedType);
    ...
    public static class EntitiesTreeUserObject<DTM extends IDomainTreeManager> extends Pair<Class<?>, String> {
        ...
        public EntitiesTreeUserObject(final DTM manager, final Class<?> root, final String property, final String firstTickCaption,//
            ...
            final Class<?> managedRoot = EntitiesTreeModel2.ROOT_PROPERTY.equals(property) ? root
                    : (manager instanceof IDomainTreeManagerAndEnhancer ? ((IDomainTreeManagerAndEnhancer) manager).getEnhancer().getManagedType(root)
                            : (manager instanceof IAbstractAnalysisDomainTreeManager ? ((IAbstractAnalysisDomainTreeManager) manager).parentCentreDomainTreeManager().getEnhancer().getManagedType(root)
                                    : root));
        ...
        private static boolean isDisabled(final ITickRepresentation tr, final Class<?> root, final String property) {
            return !EntitiesTreeModel2.ROOT_PROPERTY.equals(property) && !AbstractDomainTree.isCommonBranch(property)
                    && ((ITickRepresentationWithMutability) tr).isDisabledImmutablyLightweight(root, AbstractDomainTree.reflectionProperty(property));
        }
