...
public class ChartAnalysisModel<T extends AbstractEntity<?>> extends AbstractAnalysisReviewModel<T, ICentreDomainTreeManagerAndEnhancer ,IAnalysisDomainTreeManager, Void> {
    ...
    public ChartAnalysisModel(final AbstractAnalysisConfigurationModel<T, ICentreDomainTreeManagerAndEnhancer> configurationModel, final EntityQueryCriteria<ICentreDomainTreeManagerAndEnhancer, T, IEntityDao2<T>> criteria, final IAnalysisDomainTreeManager adtm, final PageHolder pageHolder) {
	...
	getPageHolder().addPageChangedListener(new IPageChangedListener() {
		...
		chartAnalysisDataProvider.setLoadedPage((IPage2<EntityAggregates>)e.getNewPage());
	    }
	});
    }
    ...
    public ICategoryAnalysisDataProvider<Comparable<?>, Number, IPage2<EntityAggregates>> getChartAnalysisDataProvider() {
    ...
    private static class ChartAnalysisDataProvider extends AbstractCategoryAnalysisDataProvider<Comparable<?>, Number, IPage2<EntityAggregates>> {
	...
	private IPage2<EntityAggregates> loadedPage;
	...
	@Override
	public IPage2<EntityAggregates> getLoadedData() {
	    return loadedPage;
	}
	 ...
	 * @param loadedPage
	 */
	private void setLoadedPage(final IPage2<EntityAggregates> loadedPage){
	    if(loadedPage != null ){
