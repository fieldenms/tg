...
public class DragAndMoveSpike extends ApplicationFrame implements ChartMouseListener, MouseListener, MouseMotionListener {

    public static void main(final String[] paramArrayOfString) {
        final DragAndMoveSpike seriesAndPointDragAndMove = new DragAndMoveSpike("Series & Point Dragging Demo");
    ...
    public DragAndMoveSpike(final String paramString) {
        super(paramString);
        jfreechart = ChartFactory.createTimeSeriesChart("Series & Point Dragging Demo", "Date", "Price Per Unit", createDataset(), true, true, false);
    ...
    public void mousePressed(final MouseEvent e) {
        ...
        final ChartMouseEvent cme = new ChartMouseEvent(jfreechart, e, entities.getEntity(x, y));
    ...
    public void movePoint(final MouseEvent me) {
        try {
            if (canMove) {
                ...
                if (localTimeSeries.getValue(itemIndex).doubleValue() + difference > xy.getRangeAxis().getRange().getLength()
                        || localTimeSeries.getValue(itemIndex).doubleValue() + difference < 0.0D) {
                ...
                final double targetPoint = localTimeSeries.getValue(itemIndex).doubleValue() + difference;
            ...
            if (canMove) {
                ...
                for (int i = 0; i < localTimeSeries.getItemCount(); i++) {
                    if (localTimeSeries.getValue(i).doubleValue() + difference > xy.getRangeAxis().getRange().getLength()
                            || localTimeSeries.getValue(i).doubleValue() + difference < 0.0D) {
                        initialMovePointY = finalMovePointY;
                    }
                }
                ...
                // should be as per application needs)
                for (int i = 0; i < localTimeSeries.getItemCount(); i++) {
                    final double targetPoint = localTimeSeries.getValue(i).doubleValue() + difference;
                    if (targetPoint > 10000 || targetPoint < 0) {
                        return;
                    }
                }
