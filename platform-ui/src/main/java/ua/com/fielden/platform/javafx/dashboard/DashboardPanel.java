...
public class DashboardPanel<T extends AbstractEntity<?>> extends JFXPanel {
    ...
    public class TrafficLightsCell extends TableCell<DashboardRow<T>, Integer> {
        ...
        protected void updateItem(final Integer arg0, final boolean arg1) {
            ...
            if (getIndex() <= table.getItems().size() - 1) {
                ...
                setGraphic(new TrafficLights(sentinel.getModel(), new Runnable() {
                    @Override
                    public void run() {
                        sentinel.invokeDetails(RED);
                    }
                }, //
                new Runnable() {
                    @Override
                    public void run() {
                        sentinel.invokeDetails(YELLOW);
                    }
                }, //
                new Runnable() {
                    @Override
                    public void run() {
                        sentinel.invokeDetails(GREEN);
                    }
                }));
            }
        }
    ...
    private Scene createScene() {
        ...
        sentinelCol.setCellValueFactory(new PropertyValueFactory<DashboardRow<T>, String>("sentinelTitle"));
        ...
        resultCol.setCellValueFactory(new PropertyValueFactory<DashboardRow<T>, Integer>("countOfBad"));
