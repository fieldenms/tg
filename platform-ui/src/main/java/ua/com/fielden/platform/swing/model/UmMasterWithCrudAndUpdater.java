...
public abstract class UmMasterWithCrudAndUpdater<T extends AbstractEntity<?>, C extends IEntityDao<T>> extends UmMasterWithCrud<T, C> {
    ...
    /**
     * Sets the specified entity instance and performs the owner notifications if it was provided. In cases this is not desired this method should be overridden. See
     * {@link #setEntityWithoutNotification(AbstractEntity)} for more details.
     */
    @Override
    ...
    /**
     * Method {@link #setEntity(AbstractEntity)} notifies view owner. However, a more fine-grain control might be desired. For example, if the owner should be notified only upon a
     * successful save action then notification at any other time is inappropriate. In order to implement such support, method {@link #setEntity(AbstractEntity)} needs to be
     * overridden and a call to this method is made. Additionally, method {@link #notifyActionStageChange(ua.com.fielden.platform.swing.model.UModel.ActionStage)} needs to be also
     * overridden and when a necessary event is received call <code>owner.notifyEntityChange(getEntity())</code>.
     * <p>
