...
public class Callout extends Group {
	...
	public WrapperRectangle(final Group nodeToBeWrapped, final Position position, final Callout callout) {
	    ...
	    final double transX = Position.NORTH_EAST == position || Position.SOUTH_EAST == position ? (0 - deltaX * nodeToBeWrappedExtW) : (-nodeToBeWrappedExtW + deltaX * nodeToBeWrappedExtW);
	    final double transY = Position.SOUTH_WEST == position || Position.SOUTH_EAST == position ? (0 + deltaY * nodeToBeWrappedExtH) : (-nodeToBeWrappedExtH - deltaY * nodeToBeWrappedExtH);
	...
	protected static Polygon createTriangle(final Position position, final double nodeToBeWrappedExtW, final double nodeToBeWrappedExtH, final double transX, final double transY) {
	    ...
	    poly.getPoints().addAll(new Double[] {
		    0.0, 0.0,
		    oppositeX + leftLineCoefX * dx, oppositeY + leftLineCoefY * dx,
		    oppositeX + rightLineCoefX * dx, oppositeY + rightLineCoefY * dx
		    });
	    return poly;
	}
