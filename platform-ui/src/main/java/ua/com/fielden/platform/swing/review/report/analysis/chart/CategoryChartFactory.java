...
public class CategoryChartFactory<T extends AbstractEntity<?>> implements IChartFactory<List<T>, CategoryChartTypes> {
    ...
    private CategoryPlot createPlot(final CategoryChartTypes type) {
        ...
        final FixedCategoryAxis domainAxis = new FixedCategoryAxis(distributionProperty.size() == 1 ? distributionDescriptor.getTitle(distributionProperty.get(0).toString())
                : "Categories");
    ...
    private void initToolTips(final CategoryPlot categoryPlot, final EntityDescriptor aggregationDescriptor) {
        categoryPlot.getRenderer().setBaseToolTipGenerator(new CategoryToolTipGenerator() {
            ...
            public String generateToolTip(final CategoryDataset dataset, final int row, final int column) {
                final String toolTip = "<html>" + numberFormat.format(dataset.getValue(row, column)) + " (" + aggregationDescriptor.getTitle(dataset.getRowKey(row).toString())
                        + ")<br>" //
