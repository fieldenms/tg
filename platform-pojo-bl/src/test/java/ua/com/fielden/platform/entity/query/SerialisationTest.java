...
public class SerialisationTest {
	...
	final EntityResultQueryModel<TgVehicle> original =  select(TgVehicle.class).where().prop("model.make.key").in().model(
		select(TgVehicleMake.class).where().prop("key").like().val("RR%").model()).and(). //
	...
	final EntityResultQueryModel<TgVehicle> original =  select(TgVehicle.class).where().prop("model.make.key").in().model(
		select(TgVehicle.class).where().prop("key").like().val("RR%").groupBy().prop("model.make").yield().prop("model.make").modelAsEntity(TgVehicleMake.class)).
		model();
	final EntityResultQueryModel<TgVehicle> restored = serialiseAndRestore(original);
	...
	final EntityResultQueryModel<TgVehicle> originalModel =  select(TgVehicle.class).where().prop("model.make.key").in().model(
		select(TgVehicle.class).where().prop("key").like().val("RR%").groupBy().prop("model.make").yield().prop("model.make").modelAsEntity(TgVehicleMake.class)).
		model();
