...
public class CentreConfigDuplicateActionProducer extends DefaultEntityProducerWithContext<CentreConfigDuplicateAction> {
    ...
    protected CentreConfigDuplicateAction provideDefaultValues(final CentreConfigDuplicateAction entity) {
        if (contextNotEmpty()) {
            ...
            entity.setCustomObject(getCustomObject(selectionCrit(), appliedCriteriaEntity, empty() /* update with empty saveAsName indicating default config */, of(empty()) /* update with empty uuid indicating default config */));
        }
        return entity;
    }
