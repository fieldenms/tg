...
public class WorkbookExporter {
    ...
    public static <M extends AbstractEntity<?>> SXSSFWorkbook export(final List<Stream<M>> entities, final List<Pair<String[], String[]>> propTitles, final List<List<List<DynamicColumnForExport>>> dynamicProperties, final List<String> sheetTitles, final Optional<IEntityMasterUnifiedResourceLocator> uriGenerator) {
    ...
    public static <M extends AbstractEntity<?>> SXSSFWorkbook export(final Stream<M> entities, final String[] propertyNames, final String[] propertyTitles, final Optional<IEntityMasterUnifiedResourceLocator> uriGenerator) {
        final List<T2<String, String>> propNamesAndTitles = new ArrayList<>();
        for (int index = 0; index < propertyNames.length && index < propertyTitles.length; index++) {
            propNamesAndTitles.add(t2(propertyNames[index], propertyTitles[index]));
        }
    ...
    private static SXSSFWorkbook export(final List<DataForWorkbookSheet<? extends AbstractEntity<?>>> sheetsData, final Optional<IEntityMasterUnifiedResourceLocator> uriGenerator) {
    ...
    private static <M extends AbstractEntity<?>> void addSheetWithData(final SXSSFWorkbook wb, final DataForWorkbookSheet<M> sheetData, final Optional<IEntityMasterUnifiedResourceLocator> uriGenerator) {
        ...
        sheetData.getEntities().forEach(entity -> addRow(rowIndex, entity, sheetData, wb, uriGenerator, countHyperlinks, sheet, helper, cacheShortCollectionalProps, dateCellStyle, integerCellStyle, decimalCellStyle, moneyCellStyle, dataCellStyle));
    ...
    private static <M extends AbstractEntity<?>> void addRow(
            ...
            final Optional<IEntityMasterUnifiedResourceLocator> uriGenerator,
        ...
        final var propsWithHyperlinks = determinePropsForHyperlinks(entity, sheetData, uriGenerator);
        ...
        for (int propIndex = 0; propIndex < sheetData.getPropNames().size(); propIndex++) {
            ...
            // If the current property has a URI determined for it and the total number of cells with hyperlinks is less then the supported maximum, then provide the current cell with a hyperlink.
     ...
     * @param <M>
     */
    private static <M extends AbstractEntity<?>> Map<String, String> determinePropsForHyperlinks(final M entity, final DataForWorkbookSheet<M> sheetData, final Optional<IEntityMasterUnifiedResourceLocator> uriGenerator) {
        ...
        final Optional<String> maybeMainEntityMaster = uriGenerator.flatMap(g -> g.masterUrlFor(entity));
        // If the main entity has a master, that master should be used for the hyperlinks.
        if (maybeMainEntityMaster.isPresent()) {
            final var uri = maybeMainEntityMaster.get();
            if (thisPresent) {
                propsWithHyperlinks.put("", uri);
            }
            // key or composite key members should be associated with the main hyperlink
                ...
                Finder.getKeyMembers(entityType).stream().forEach(field -> {
                    ...
                    propsWithHyperlinks.put(keyMemberPropName, uri);
                    ...
                    if (EntityUtils.isEntityType(field.getType()) && EntityUtils.isCompositeEntity((Class<? extends AbstractEntity<?>>) field.getType())) {
                        ...
                        Finder.getKeyMembers(fieldTypeAsEntity).stream().forEach(subKeyMemberField -> propsWithHyperlinks.put(keyMemberPropName + "." + subKeyMemberField.getName(), uri));
                    }
                });
            }
        }
            ...
            Finder.getKeyMembers(entityType).stream()
                  .forEach(field -> {
                      if (EntityUtils.isEntityType(field.getType()) && !entity.proxiedPropertyNames().contains(field.getName())) {
                          ...
                          uriGenerator.flatMap(g -> g.masterUrlFor(keyMemberValue)).ifPresent(uri -> {
                              final var keyMemberPropName = field.getName();
                              propsWithHyperlinks.put(keyMemberPropName, uri);
                              // key members can be composite themselves, which would most likely result in their key members present in the result set
                              // if that is the case, we should associate those key members with the URI also
                              if (EntityUtils.isEntityType(field.getType()) && EntityUtils.isCompositeEntity((Class<? extends AbstractEntity<?>>) field.getType())) {
                                  final Class<? extends AbstractEntity<?>> fieldTypeAsEntity = (Class<? extends AbstractEntity<?>>) field.getType();
                                  Finder.getKeyMembers(fieldTypeAsEntity).stream().forEach(subKeyMemberField -> propsWithHyperlinks.put(keyMemberPropName + "." + subKeyMemberField.getName(), uri));
                              }
