 ...
 * <li>If node represents token NAME (i.e. property name in dot notation) then its level is the number of collectional properties in the value plus 1. For example, if WorkOrder is
 * the context then properties <code>totalCost</code>, <code>vehicle.techDetauls.doorsQty</code> have the level value of 1; at the same time properties
 * <code>vehicle.fuelUsages.qtyPurchased</code> have the level value of 2 due to one collectional property <code>fuelUsages</code> in the path.
 * <li>If node represents PLUS, MINUS, MULT, DIV, DAY, MONTH, YEAR, UPPER, LOWER, DAY_DIFF then its level is the same as for its operands; if the operands have different levels
 * then an exception is thrown; it is possible to have a mix of level agnostic operand and an operand with some level assigned; this way the level of the node should be assigned to
 * the level of the operand with the level value defined; if both operands are level agnostic then the node becomes also level agnostic.
 * <li>If node represents aggregation functions such as AVG, SUM, MIN, MAX or COUNT then its level value is the level of its operand - 1 or <code>null</code> if the operand is
 * level agnostic.
 * </ul>
 * <p>
 ...
 * For example, if there is an entity <code>WorkOrder</code>, which properties <code>cost: Money</code> and <code>veh: Vehicle</code>, and there is a need to use property
 * <code>cost</code> as part of a calculated property in the context of property <code>veh</code>, then it needs to be prefixed with <code>←.</code>. In this case,
 * <code>WorkOrder</code> is considered as a higher-order type, and its property <code>veh</code> -- a context property.
 * 
 * @author TG Team
 * 
 */
public class LevelAllocatingVisitor extends AbstractAstVisitor {
    ...
    /**
     * Iterates over node's children in order to find the first one with level value assigned. Compares the found value with levels of the rest of children to ensure that they're
     * equal or level agnostic.
     * 
     * @param node
     * @return -- the level to be assigned to the passed in node.
     * @throws SemanticException
     *             -- indicates level incompatibility between node's children.
     */
