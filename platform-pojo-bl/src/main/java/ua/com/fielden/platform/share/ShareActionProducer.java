    ...
    protected ShareAction provideDefaultValues(final ShareAction entity) {
        if (contextNotEmpty()) {
            ...
            ofNullable((String) getContext().getCustomObject().get(CUSTOM_OBJECT_SHARED_URI)).ifPresentOrElse(sharedUri -> {
                ...
                final var tinyHyperlink = coTinyHyperlink.saveWithTarget(new Hyperlink(sharedUri), Optional.of(fetchOnly(TinyHyperlink.class).with(HASH))).asRight().value();
                ...
                if (masterEntityNotEmpty()) {
                    ...
                    final TinyHyperlinkCo coTinyHyperlink = co(TinyHyperlink.class);
                    final var tinyHyperlink = coTinyHyperlink.save(masterEntity.getType(), savingInfoHolder, IActionIdentifier.of(actionIdentifierName), Optional.of(fetchOnly(TinyHyperlink.class).with(HASH))).asRight().value();
