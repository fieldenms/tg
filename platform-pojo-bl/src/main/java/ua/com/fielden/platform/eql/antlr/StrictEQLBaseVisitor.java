...
/// A stricter alternative to [EQLBaseVisitor] that should be used instead.
/// Unlike [EQLBaseVisitor], whose visiting methods silently fall through via `visitChildren`, visiting methods of this class throw [EqlVisitorException].
/// This helps prevent accidental mistakes when implementing EQL visitors.
///
/// ### Rationale
///
/// Concrete visitors typically need only a subset of all visiting methods (e.g., a visitor of operands deals only with operands).
/// However, given the design of ANTLR visitors, it is not possible to ensure, at compile time, that all _relevant_ visiting methods are implemented by the visitor.
/// Therefore, this kind of validation has to be performed at runtime.
///
/// ### Maintenance
///
/// This class was originally derived from the ANTLR-generated [EQLBaseVisitor], but it is **not** regenerated automatically.
/// When the EQL grammar changes and ANTLR adds new `visit*` methods to [EQLVisitor], corresponding methods must be added here manually, each delegating to [#defaultVisit()].
/// Failing to do so will result in a compilation error, since this class implements [EQLVisitor].
///
...
public abstract class StrictEQLBaseVisitor<T> extends AbstractParseTreeVisitor<T> implements EQLVisitor<T> {
    ...
    /// Default implementation of visiting methods. Guards subclasses from calling unimplemented visitor methods by throwing a runtime exception.
    ///
