...
public interface ISearchAlgorithm<T, N extends ITreeNode<T>> {
    /**
     * Accepts the root node and the predicate to be satisfied. Return the first predicate satisfying node.
     * If the whole tree needs to be traces in order to visit all of its nodes then predicate implementation should always return <code>false</code>.
     * <p>
