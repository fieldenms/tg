...
abstract class AbstractConditionalOperand<T1 extends IComparisonOperator<T2>, T2 extends ILogicalOperator<?>> extends AbstractExpConditionalOperand<T1> implements IComparisonOperand<T1>, IExistenceOperator<T2> {
    ...
    @Override
    public <E extends AbstractEntity> T2 exists(final UnorderedQueryModel subQuery) {
	getTokens().exists(false, subQuery);
	return getParent2();
    }

    @Override
    public <E extends AbstractEntity> T2 notExists(final UnorderedQueryModel subQuery) {
	getTokens().exists(true, subQuery);
	return getParent2();
    }

    @Override
    public <E extends AbstractEntity> T2 existsAnyOf(final UnorderedQueryModel ... subQueries) {
	getTokens().existsAnyOf(false, subQueries);
	return getParent2();
    }

    @Override
    public <E extends AbstractEntity> T2 notExistsAnyOf(final UnorderedQueryModel ... subQueries) {
	getTokens().existsAnyOf(true, subQueries);
	return getParent2();
    }

    @Override
    public <E extends AbstractEntity> T2 existsAllOf(final UnorderedQueryModel ... subQueries) {
	getTokens().existsAllOf(false, subQueries);
	return getParent2();
    }

    @Override
    public <E extends AbstractEntity> T2 notExistsAllOf(final UnorderedQueryModel ... subQueries) {
