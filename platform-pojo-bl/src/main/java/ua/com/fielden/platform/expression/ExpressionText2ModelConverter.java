...
/**
 * Binds together expression lexer, parser and AST visitors to validate an expression from both syntactic and semantic perspectives and to produce a computational model used as
 * part of the Entity Query API.
 * 
 * @author TG Team
 * 
 */
public class ExpressionText2ModelConverter {
     ...
     * The <code>contextProperty</code> argument specifies the relative location in the type tree against which all other AST nodes' levels should be checked for compatibility. The
     * value of the <code>contextProperty</code> argument should be null if the higher-order type represents a context.
     * 
     * @param higherOrderType
     * @param contextProperty
     * @param expressionText
     *            -- a textual representations of the expression, which gets parsed and validated.
     */
    ...
    /**
     * Performs syntactic, semantic analysis and produces a computational model, which can be used for entity query API. Returns the root AST node in case of successful validation
     * and model creation. The computational model can be accessed by invoking method <code>getModel()<code> on the root node.
     * 
     * @return
