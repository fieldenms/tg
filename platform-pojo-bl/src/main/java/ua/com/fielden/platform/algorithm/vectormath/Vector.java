...
/**
 * A simple vector representation with known math operations (like add, subtract, cross product etc) in Cartesian coordinate system.
 * 
 * @author TG Team
 * 
...
public class Vector {
     ...
     * <p>
     * <code>this</code> is a primary vector (right-handed forefinger) and <code>that</code> is a secondary vector (right-handed middle finger) and a result vector (right-handed
     * thumb).
     * <p>
     * If result is equal zero -- two vectors are parallel (or even collinear). If result is lower than zero then angle between vectors is in (180; 360) range. If result is upper
     * than zero then angle between vectors is in (0; 180) range.
     * 
     * @param that
     * @return
     */
    public BigDecimal crossProduct(final Vector that) {
        return sc((this.x.multiply(that.y)).subtract(this.y.multiply(that.x)));
    }
