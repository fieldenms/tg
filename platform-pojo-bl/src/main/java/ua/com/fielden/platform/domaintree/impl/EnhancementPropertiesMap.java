...
public class EnhancementPropertiesMap<T> extends HashMap<Pair<Class<?>, String>, T> {
    ...
    public T put(final Pair<Class<?>, String> key, final T value) {
        return super.put(pair(DynamicEntityClassLoader.getOriginalType(key.getKey()), key.getValue()), value);
    }

    @Override
    public T get(final Object key) {
        final Pair<Class<?>, String> key1 = (Pair<Class<?>, String>) key;
        return super.get(pair(DynamicEntityClassLoader.getOriginalType(key1.getKey()), key1.getValue()));
    }

    @Override
    public T remove(final Object key) {
        final Pair<Class<?>, String> key1 = (Pair<Class<?>, String>) key;
        return super.remove(pair(DynamicEntityClassLoader.getOriginalType(key1.getKey()), key1.getValue()));
    }
}
