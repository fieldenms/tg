...
public class VectorIntersector {
     ...
     */
    public static Pair<Boolean, Pair<BigDecimal, BigDecimal>> intersect(
	    final BigDecimal x0, final BigDecimal y0, 
	    final BigDecimal X0, final BigDecimal Y0, 
	    final BigDecimal x, final BigDecimal y, 
	    final BigDecimal X, final BigDecimal Y) {
	...
	final BigDecimal t = qSubP.crossProduct(s).divide(crossRandSProduct);
	final BigDecimal u = qSubP.crossProduct(r).divide(crossRandSProduct);
	if (0.0 <= t.doubleValue() && t.doubleValue() <= 1.0 && 
		0.0 <= u.doubleValue() && u.doubleValue() <= 1.0) {
