...
@Internal
public class TgLambdaFetchingSupport {
     ...
     */
    public static Optional<Function<Object, Object>> createGetter(final Class<?> sourceClass, final String propertyName) {
        final Method candidateMethod = getCandidateMethod(sourceClass, propertyName);
    ...
    private static Method getCandidateMethod(final Class<?> sourceClass, final String propertyName) {
        ...
        final List<Method> allGetterMethods = findMethodsForProperty(sourceClass,
                getterPredicate);
        final List<Method> pojoGetterMethods = allGetterMethods.stream()
                .filter(TgLambdaFetchingSupport::isPossiblePojoMethod)
                .collect(toList());
    ...
    private static Method checkForSingleParameterPeer(final Method candidateMethod, final List<Method> allMethods) {
        // getFoo(DataFetchingEnv ev) is allowed, but we don't want to handle it in this class
        // so this find those edge cases
        for (final Method allMethod : allMethods) {
     ...
     */
    private static List<Method> findMethodsForProperty(final Class<?> sourceClass, final Predicate<Method> predicate) {
    ...
    private static boolean isPossiblePojoMethod(final Method method) {
    ...
    private static boolean isRecordLike(final Method method) {
        return !isObjectMethod(method) &&
                returnsSomething(method) &&
                hasNoParameters(method) &&
                isPublic(method);
    ...
    private static boolean isGetterNamed(final Method method) {
        final String name = method.getName();
        return ((name.startsWith("get") && name.length() > 4) || (name.startsWith("is") && name.length() > 3));
    }

    private static boolean returnsSomething(final Method method) {
    ...
    private static String mkPropertyNameGetter(final Method method) {
        //
    ...
    @VisibleForTesting
    static Function<Object, Object> mkCallFunction(final Class<?> targetClass, final String targetMethod, final Class<?> targetMethodReturnType) throws Throwable {
        final MethodHandles.Lookup lookup = getLookup(targetClass);
        final MethodHandle virtualMethodHandle = lookup.findVirtual(targetClass, targetMethod, MethodType.methodType(targetMethodReturnType));
    ...
    private static MethodHandles.Lookup getLookup(final Class<?> targetClass) {
        final MethodHandles.Lookup lookupMe = MethodHandles.lookup();
        //
