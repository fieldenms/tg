...
public abstract class GroupAnalysisQueryGenerator<T extends AbstractEntity<?>> implements IReportQueryGenerator<T> {
    ...
    public GroupAnalysisQueryGenerator(final Class<T> root, final ICentreDomainTreeManagerAndEnhancer cdtme, final IAbstractAnalysisDomainTreeManager adtm, final IUniversalConstants universalConstants) {
        this.root = root;
        this.cdtme = cdtme;
        this.adtm = adtm;
    ...
    protected IQueryComposer<T> createQueryAndGroupBy(final Class<T> genClass, final List<String> distributionProperties) {
        ...
        final EntityResultQueryModel<T> subQueryModel = DynamicQueryBuilder.createQuery(managedType, ReportQueryGenerationUtils.createQueryProperties(getRoot(), getCdtme()), universalConstants).model();
        ...
        final Map<String, Pair<Object, Object>> paramMap = EntityQueryCriteriaUtils.createParamValuesMap(getRoot(), managedType, getCdtme().getFirstTick(), universalConstants);
