 ...
 * returned state should be used for further input processing by invoking its method {@link AbstractState#accept(char)}.
 * <p>
 * Each state specified for an instance of automata maintains a back link, to ensure easy tracking of the current state, the last processed symbol and the processed sequence using
...
public class NonDeterministicAutomata {
    ...
    /**
     * Attempts to recognise a token from the beginning of the input text. The recognitions may result in failure or a complete/partial recognition of the input text.
     * 
     * @param input
     *            -- the input text, which is processed.
     * @param posInOriginalSequence
     *            -- the starting positions of the input text in the original sequence (input is usually a substring of some other character sequence).
