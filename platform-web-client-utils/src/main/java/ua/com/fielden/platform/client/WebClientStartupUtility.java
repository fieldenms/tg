...
public class WebClientStartupUtility {
                    ...
                    final StyledLoginScreenModel loginScreenModel = new StyledLoginScreenModel(authenticationModel, "Login Screen", ResourceLoader.getIcon("images/styled-login-scr-background.png")) {
                        ...
                        protected void authenticationPassed(final StyledLoginScreen loginScreen, final Result result) {
                            ...
                            if (Workflows.development.equals(Workflows.valueOf(settings.workflow())) && !user.isBase()) {
                                throw new IllegalArgumentException("Development mode of the client application startup is not permitted for non-base user. Please start the client application in deployment mode. Current user ["
                                        + user + "] is not base user.");
                            }
                            try {
     ...
     * @param injector
     * @param emm
     *            -- entity master manager, which provides instantiation and caching of entity master review/edit windows.
     * @param mmBinder
     *            -- main menu binder, which is used for instantiation and configuration of the application main menu.
     * @param caption
     *            -- default application caption displayed in the title of the main window.
     * @param dim
     *            -- default main window dimension
     * @param icon
     *            -- application icon, which appears in the title of every application window.
     * @param logger
     */
