...
//        modelAsEntity(type);
//        page = rao.firstPage(from(model).with(fetchAll(type)).model(), from(summaryModel).with(fetchAll(type).without("intPropertySum").without("minimaxDiff")).model(), 15, binaryTypes);
    }
