...
/**
 * Represents a web resource mapped to URI /export/entity-alias-type. It handles POST requests provided with {@link IQueryOrderedModel} and a list of properties with their
...
public class EntityQueryExportResource<T extends AbstractEntity> extends Resource {
	...
	try {
	    ...
	    final IQueryOrderedModel<T> query = (IQueryOrderedModel<T>) list.get(0);
	    final fetch<T> fetchModel = (fetch<T>) list.get(1);
