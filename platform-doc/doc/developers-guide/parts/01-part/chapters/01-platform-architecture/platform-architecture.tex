...
public class TonnageValidator 
  ...
  \begin{notebox}{Defensive or not.}{\label{nb:DefensiveOrNot}}
    ...
    Althoug, it might be a good idea in general, our past experience and analyses suggests that the meaning of \emph{defensive} should always be treated within a specific semantic context.\\
    ...
    Alternatively, there could be a check that fails validation if \emph{newValue} is \emph{null}, which has only one benefit over no check at all -- it could provide a more meanigful error message stating that property value is required.
