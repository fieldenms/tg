...
\subsection{Enhance Person and Test Your Changes}
  ...
  Type \texttt{tgp} and hit \emph{Ctrl+Space} in the code editor of type Person under the last available property.
  ...
  Thus, in the wizard change the package value from \texttt{fielden.personel} to \texttt{fielden.personel.validation}.
  ...
  The test-first approach works really well when implementing business logic, which ensures both the correctness of the business logic and the quickest possible implementation time by navigating the development effort along the requirements captured tests.
...
public class BirthdayValidator implements IBeforeChangeEventHandler<Date> {
  ...
  Open test case \texttt{PersonnelTest} by using the \emph{Open Type} dialog as before in the editor, and without explaining too much of the existing code add the two tests from Listings~\ref{lst:Person-birthday-validation-test-1} and ~\ref{lst:Person-birthday-validation-test-2}.

 \begin{code}{Personnel younger 23 should produce warning}{\label{lst:Person-birthday-validation-test-1}}{codebgcolor}
    \begin{lstlisting}
@Test
public void birthday_should_produce_warning_and_be_saved() {	
  final Person person = ao(Person.class).
			findByKey("USER2").
			setBirthday(date("2000-01-01 10:10:00"));	
  ...
  assertEquals("Should have been saved.", 
	       date("2000-01-01 10:10:00"), 
               ao(Person.class).findByKey("USER2").getBirthday());
}
    \end{lstlisting}
  \end{code}

  Both test rely on the fact that test data contains a person with business key equals to \emph{USER2} and property \texttt{birthday} that has no value (i.e. it is null).
  The first test ensures rule \emph{2.a} that if the birth date suggests a person younger than 23 years of age then a warning should be raised, but the value is recognised as valid and saved when requested.
  The second test ensures rule \emph{2.b} that no future birth dates are acceptable, and thus are not saved.
  Running these tests by pressing \texttt{Ctrl+F11} combination should result in failure of both tests\footnote{One existing test should succeed.}.
  This means we need to keep coding the validation rules until both of these tests are successful.
  Before moving further in the text, please try to implement the validation rules to make the tests pass.
  
\begin{code}{Unborn personnel is not acceptable}{\label{lst:Person-birthday-validation-test-2}}{codebgcolor}
    \begin{lstlisting}
@Test
public void birthday_should_produce_error_and_not_be_saved() {	
  final Person person = ao(Person.class).
			findByKey("USER2").
 ...
 Listing~\ref{lst:Person-birthday-validation} provides a complete validation logic that satisfies all the requirements.
...
public class BirthdayValidator implements IBeforeChangeEventHandler<Date> {
	...
	if (newValue != null) {
	    ...
	    // warn is younger than 23 years of age 
  ...
  The next step is to observe our changes in the actual application.
