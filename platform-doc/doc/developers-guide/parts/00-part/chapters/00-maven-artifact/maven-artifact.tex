...
%\myepigraph{In our profession, precision and perfection are not a dispensable luxury, but a simple necessity.}{Niklaus Wirth}
  ...
  For example, if the file name of some dependency is \texttt{platform-pojo-bl-\tikzinline{1.1}.jar} then the highlighted portion \texttt{\tikzinline{1.1}} indicates the version, and the preceding part \texttt{platform-pojo-bl} is the artifact ID.
  ...
  \begin{image}{A fragment of the local repository structure}{\label{img:ch00:01:local_repository_structure}}    
    ...
    \tikznote{tree_a1}{settings}{6cm}{-0.2cm}{4cm}{Settings File}{Contains Maven specific settings, which are discussed in details later in this section.}
  \end{image}
