...
\subsection*{What is Trident Genesis?}
  Trident Genesis (TG) is an opinionated software platform for development of software systems in a domain driven way.
...
\subsubsection*{Unique object-oriented architectural style to uniform modelling of the business domain}
	One could argue that commanding a programming language is relatively easy. 
  	...
  	This protects developers from multitude of programming errors provisioning for getting a working solution in short terms that could be easily supported by both the original or new developers.
...
\subsubsection*{Single programming language (Java)}
	...
	Databases require the SQL knowledge, constructing Web UI requires the knowledge of HTML, CSS and JavaScript, the middleware requires some other language such as Java, Scala or C\# as well as associated frameworks.
	...
	And this single language is Java, one of the currently most popular programming languages.
	Application developers that know Java can use TG to concentrate on building a core business value and deliver scalable and responsive software solutions without delving deep down into the underlying technology stack.
	The developers can keep using their favourite IDE and take advantage of interactive debugging and static code analysis tools.
...
\subsubsection*{Automated business domain model verification by means of domain driven testing}
	...
	The TG platform provides the first-class support for unit and functional testing in a domain driven way.
...
\subsubsection*{Continuous validation and integration of the business domain models during the system lifecycle}
	The TG platform stands on the shoulders of the giants.
	This is no different when it comes to continuos integration (CI) and validation of TG-based software systems.
	The domain driven testing is fully compatible with modern CI tools like Jenkis and Hudson.
...
\subsubsection*{Direct interaction and manipulation of the business domain model}
	...
	This significantly reduces the misconceptions about the system's actions, structure and logic, and empowers the users to truly utilise the capabilities of the business domain model to its fullest capabilities.
	Instead of using external tools for accessing raw data in underlying databases, the users are provided with an embedded business intelligence capability to interact with the data though a well defined business domain model.
	\begin{figure}[!h]
  		\centering
...
\subsubsection*{Responsive HTML5 clients}
	...
	The UI concepts are all based on the business domain modelling principles to satisfy the worldview metaphor, and follow the Google Material Design spec to delivery high quality, responsive and consistent UX across devices of multiple form factors.
...
\subsubsection*{A software construction tool to facilitate the process management}
	...
	These project management approaches exist for a good reason and there are multitude of tools that assists with their implementation and practices at business organisations.
	In our experience, this is only one side of the medal.
	...
	This results in having the software construction tools that supports the chosen method for process management.
...
\subsubsection*{Cloud ready}
	...
	The platform employs RESTfull architecture to structure application web-resources.
...
\subsubsection*{Complete application development lifecycle}
	...
	Thus, the development of software systems with TG covers all what is required for end-to-end software construction.
