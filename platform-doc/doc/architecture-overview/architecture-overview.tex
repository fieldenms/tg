\documentclass[a4paper,12pt,final]{hitec}
\usepackage[english]{babel}
\usepackage{etex}
\usepackage{times}
\usepackage{setspace}
\usepackage{inputenc}
\usepackage{amssymb}
\usepackage{amsfonts}
\usepackage{amsmath}
\usepackage{graphicx}%[pdftex]
\usepackage{wrapfig}
\usepackage{subfig}
\usepackage{alltt}
\usepackage{moreverb}
%for more info on hyperref package see http://en.wikibooks.org/wiki/LaTeX/Packages/Hyperref
%\usepackage[pdftex,colorlinks=true,linkcolor=blue]{hyperref}
\usepackage{eso-pic}
% tikz related packages to provide scalable graphics 
\usepackage{tikz}
\usetikzlibrary{mindmap,backgrounds,positioning,arrows,shapes,shapes.arrows,shapes.misc,automata,petri,patterns,scopes,chains,matrix,decorations.pathmorphing,shadows,calc}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%% Backgound picture %%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newcommand\BackgroundPic{
\put(-15,0){
\parbox[b][\paperheight]{\paperwidth}{%
\vfill
\centering
  \includegraphics[width=1.1\paperwidth,height=1.1\paperheight, keepaspectratio]{sections/00-toc/images/background}%
\vfill
}}}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%% progrma code formattin environment %%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\usepackage{listings}
\usepackage{float}
\usepackage{caption}
%allows use of "@" before \begin{document}
\makeatletter
% this creates a custom and simpler ruled box style
\newcommand{\floatc@simplerule}[2]{{\@fs@cfont #1 #2}\par}
\newcommand\fs@simplerule{\def\@fs@cfont{\bfseries}\let\@fs@capt\floatc@simplerule
  \def\@fs@pre{}%\hrule height.4pt depth0pt \kern4pt
  \def\@fs@post{}% \kern4pt\hrule height.4pt depth0pt \kern4pt \relax
  \def\@fs@mid{}% \kern4pt
  \let\@fs@iftopcapt\iftrue}

% this code block defines the new and custom floatbox float environment
\floatstyle{simplerule}
\newfloat{program}{thp}{lop}
\floatname{program}{Code Snippet}
\definecolor{codebgcolor}{HTML}{FFF3BC}
\definecolor{dkgreen}{rgb}{0,0.3,0}
\definecolor{gray}{rgb}{0.5,0.5,0.5}
\definecolor{mauve}{rgb}{0.58,0,0.82}
\newenvironment{code}[2]
{\begin{program}
  \vspace*{5pt}
  \captionsetup{format=plain,justification=centering,labelsep=colon,labelfont=bf,singlelinecheck=false,indention=0pt,position=below}
  \caption{#1}\label{#2}  
  \begin{center}
    \begin{tikzpicture}
      \node [fill=dkgreen!10!white,rounded corners=5pt]
      \bgroup
      \bgroup
      \begin{tabular}{l}}
      {\end{tabular}
      \egroup
      \egroup;
    \end{tikzpicture}
  \end{center}
\end{program}
}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%



\begin{document}
 %\AddToShipoutPicture*{\BackgroundPic}
  \setstretch{1.2}
  \include{sections/00-toc/toc} % this file contains the title page and the abstract  
  \setcounter{page}{4} % need to reset page counter as by default after toc it start with 1 again 
  % using \input insterad of \include in order not statr every section from new page
  \input{sections/01-intro/intro} %
  \input{sections/02-platform-and-business-applications/platform-and-business-applications} %
  \input{sections/03-dsl-types-metadata/dsl-types-metadata} %
  \input{sections/04-data-management-and-web-resources/data-management-and-web-resources} %
  \input{sections/05-user-interface/user-interface} %
  \input{sections/06-analytical-and-reporting-capabilities/analytical-and-reporting-capabilities} %
  \input{sections/07-deployment-and-versioning/deployment-and-versioning} %
  \input{sections/08-conclusion-and-future/conclusion-and-future} %
\end{document}