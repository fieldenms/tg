...
\section*{Secure communication and authentication}
...
\subsubsection*{Communication over HTTPS}
	...
	This leads to two possible ways for an adversary to access the passwords. Firstly, the application source might get stolen (e.g. a GitHub repo gets hacked).
	...
	The most reasonable option is to provide those password as an application server argument during its start up.
	Even in case of keeping them in a start up script on the deployment server would be much safer than keeping them in a class file (source or compiled).

	In addition, different TLS certificates should be used for development and deployment purposes, and kept in different keystore files.
