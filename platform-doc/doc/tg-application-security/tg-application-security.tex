...
\section*{Configuration management}\label{sec:04}
	...
	Running software as containers such as Docker, offers an alternative approach that in many respects is simpler and more reliable.
...
\end{enumerate}
	...
	The first Docker container~(5) represents a reverse proxy and a load balancer such as HA Proxy.
	...
	Running instances of TG application servers behind HA Proxy that is both a load balancer and TLS termination proxy, provides an easy way to scale the deployment up when and if required by spinning off new Docker containers.
	...
	There are several approaches how these settings can be passed to the Docker container.
