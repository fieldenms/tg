...
{\textcolor{basiccolor}{\Huge A framework for RESTful Objects}}\\[\baselineskip]
{\small a software engineering perspective}\par
\vfill
...
\usepackage{hyperxmp}
\hypersetup{
    pdftitle={Trident Genesis Platform: a framework for RESTful Objects},
    pdfauthor={Fielden Management Services Pty. Ltd.},
    pdfsubject={A hight level overview of key principles and advantages of the RESTful Objects architectural pattern and a framework that implements it.},
...
\subsection*{The REST of Object-Oriented Programming}	
	...
	Unlike the anemic domain model~\cite{fowler2003}, an entity has the responsibility to ensure the correctness of its state\footnote{This is not unlike ``design by contract'', introduced by Bertrand Meyer~\cite{bm1997}, where integrity constraints are \emph{pre-conditions}.}, which is expressed through integrity constraints.
	...
	Without going any further into details, it is important to emphasise that in real world systems, the validation process rarely operates only on the in-memory state and often requires access to a database.
	The developed entity validation mechanism is specifically designed with this requirement in mind\footnote{Constraint to data retrieval only and offering no data mutation capability.} and does not follow an approach that is frequently used in simpler problem domains where property setters are used to place the validation logic.
	Instead, validators are represented by separate classes\footnote{Validators can be represented by interfaces, which concrete implementations resolved at runtime through IoC.} that are implemented as a property \emph{before change} contract and are executed by the \texttt{TG} runtime before setter invocations.
	...
	The meta-data is not limited only to identifying erroneous entity properties and their causes, but it also captures and maintains in-memory mutations such as the aforementioned attempted values, previous and original values.
...
private Service service;
	...
	\begin{figure}[!h]
  		\centering
        \includegraphics[width=0.8\textwidth]{images/01-wa-master.png}  
	...
	Due to this successful assignment and the declared dependency, property \texttt{locationSubsystem} in the same entity had to be revalidated as it could have become invalid.
	However, the revalidation process did not fail~-- it resulted in a warning.
	This shows that the validation result does not have to be binary~-- success or failure -- there are many situation where a value is acceptable, but users need to be informed of some consequential information.
	In this particular example, the user is informed that there are already open work activities for this location/subsystem.	
	
	\begin{figure}[!h]
  		\centering
      	\includegraphics[width=1.0\textwidth]{images/03-wa-dependent-props-warning.png}  
	...
	If a user reports a defect, for example, stating that some value should have been excepted, but wouldn't due to some validation error, the developers know exactly where to look for a potential cause of an error\footnote{This is the direct manipulation principle in action, significantly enhancing the capability for informal reasoning about a system.}.
	...
	\vspace{8pt}
	In addition to property \emph{validators}, there is also support for, what we call, property \emph{definers}\footnote{Not to be confused with \emph{post-conditions}, which in TG-based systems is usually expressed as domain unit tests.}.
...
private Technician technician;
	...
	\tcblower
		\tiny
		Property \texttt{technician} of type \texttt{Technician} in entity \texttt{WorkActivity} is declared as persistent and required, which means it cannot be empty and gets saved into a database.
	...
	Figure~\ref{fig:tech} illustrates an entry of value \texttt{TJW} into property \texttt{technician}.
	...
	\begin{figure}[!h]
  		\centering
      	\includegraphics[width=0.6\textwidth]{images/04-wa-technician-entry.png}
	...
	Figure~\ref{fig:derived-manager} depicts the result of a successful assignment of value \texttt{TJW} to property \texttt{technician}, which resulted in an auto-assignment of a derived value \texttt{MPC} to property \texttt{manager}.  

	\begin{figure}[!h]
  		\centering
      	\includegraphics[width=0.6\textwidth]{images/05-wa-technician-entered.png}  
	...
	It represents a RESTful interface to its counterpart -- \emph{entity object}.
	...
	For example, with such approach entity purchase order could be modelled as class \texttt{PurchaseOrder} with separate methods to print a purchase order, export a purchase order, add a purchase order line, authorise a purchase order, etc.
	...
	There is no way to bypass the validation logic when changing an entity and there is no other way than to use a corresponding companion object to save new or modified entity instances.
	...
	Such uniformity contributes significantly to the comprehension of an information system by both the original and new developers that just need to follow the established pattern to identify how different parts of the system are composed, interact with each other and what are the rules for that interaction.
	...
	There can be multiple entity instance of this type associated with the same work activity and any existing detail can be edited under some conditions.
		
	\begin{figure}[!h]
  		\centering
        \includegraphics[width=0.6\textwidth]{images/06-wa-detail-master.png}  
        \caption{Work Activity Detail}
        \label{fig:wa-detail}
		%\vspace{-30pt} 
  	\end{figure}

	Entity \texttt{WorkActivityDetail} has four properties.
	Three of those are read-only from UI perspective and can only be changed programmatically, the remaining one is required\footnote{This nature of properties is automatically taken into account in the UI layer by reusing the definitions that are captured at the domain model.}.
	Buttons \texttt{CANCEL} and \texttt{SAVE} are directly mapped to the two of four companion object operations -- \texttt{retrieve}\footnote{Cancelling changes for a persistent entity is equivalent to refreshing an entity from a database by means of re-retrieving it.} and \texttt{save}.
	...
	Entity retrieval in this example is quite trivial and its only concern is to use an appropriate \emph{fetch model} to obtain an object subgraph that is required for editing purposes.
...
public class WorkActivityDetailCo 
	...
	In this case, it makes sense to call \texttt{super.save} as the last method instruction (line 19) in order to persist preceding entity changes (lines 15 and 16).
	...
	Lines 11-17 correspond to a business logic, which dictates that under certain condition properties \texttt{date} and \texttt{person} of \texttt{detail} (the entity being saved) should be updated with the current date/time and the currently logged-in person, who actions the change.
	The condition (lines 11 and 13) to update values for these properties checks if \texttt{detail} has a timesheet associated with it and if its property \texttt{comment} is dirty.
	The latter is an example of using meta-data API to identify if some property was changed, but not yet persisted.

	Entity properties are updated by invoking property setters (lines 15-16).
	...
	Property \texttt{person} is assigned the value of \emph{current person} by obtaining an instance of a companion object for entity \texttt{Person} (i.e., \texttt{co(Person.class)}) and then calling its method \texttt{currentPerson()}.
	...
	Entity \texttt{User} with its companion \texttt{UserCo} are present at the platform level and can be accessed by the application specific domain logic.
	...
	That is why it is possible and more natural from the domain perspective to express the notion of a currently logged-in user in terms of domain entity \texttt{Person}.
	...
	It is always clear what changes need to be implemented at the domain model level whenever an application user refers to some functionality in terms of UI artifacts.
	
	So far we have only touched on how the the RESTful Objects pattern can be used to model persistent entities (i.e., those that correspond to the \texttt{Memory} function of an information system).
	The next section delves more into the application of the RESTful Objects pattern to model the \texttt{Informative} and \texttt{Active} functions.
	...
	We also introduce Entity Query Language (EQL) as an essential part for expressing database queries in a reliable model-driven way.
	EQL is used to construct \emph{Synthetic Entities} based on sophisticated queries that capture the information across multiple \emph {Persistent Entities}, while following the same RESTful Objects pattern.
	
	Last, but not least, we will discuss declarative domain security and how RESTful Objects deliver GraphQL-based Web API by following the direct manipulation principle in the same way as with UI.\\[100pt]


	{\noindent\HUGE Work in progress\ldots}
...
\iffalse
	...
	REST, on the other hand, operates on resource representations, each one identified by an URL. These are typically not data structures, but complex object abstractions.
...
\section*{The problem statement}
	...
	The unique trait of the \emph{RESTful Objects} pattern is in establishing a uniform approach for designing and implementing the three core functions of any information system~-- \emph{memory}, \emph{informative}, \emph{active}~\cite{oli2007}~-- by directly expressing Structural Schema, Integrity Constraints and Behavioural Schema at the level of objects.
...
\section*{What is Trident Genesis?}
  ...
  \vspace{3 mm}
  \noindent The TG platform incorporates over 30 years of experience in delivering Enterprise grade software products at a software architecture level.
