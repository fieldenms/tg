...
\section*{The architectural pattern of TG}
	...
	Taken literally, this perspective leads to many issues, including violation of SOLID principles that have, effectively, come to existence because them.
	...
	Naturally, it exposes object properties for users to change them directly via UI, but in order to provide some behaviour, it exposes some of the methods as UI actuators (e.g. buttons, menu items) that can be invoked on an individual object.
	The approach of exposing object methods, introduces new issues and opens up many questions.
	For example:
  	\begin{itemize}
    	\item There is now need to differentiate between methods (i.e. behaviour) that can and cannot be exposed via UI;
	...
	This suggests that the described problem of having no first-class support for verbs, significantly complicates, if not makes it impossible, to construct semantically transparent information systems by following the currently most prevalent understanding of object orientation.

\subsection*{Verbs == Nouns}	
	
	In our research we came across some original work by Alan Kay where he explains object orientation from a very different that currently taken perspective.
	According to Alan Kay, the complexity of behaviour in object-oriented systems comes from interconnectedness between objects rather than from methods, which they have.
	He compared an object-oriented systems with a systems of networked computers, or computing nodes, that have simple communication interface for interaction.
	This and the rise of RESTful concepts resonated 
	
	The lack of seemingly no first-class support for verbs in OOP languages
	In our
...
\bibitem{fowler2006} Fowler, M. DslBoundary, http://martinfowler.com/bliki/DslBoundary.html

\bibitem{yegge2006} Yegge, S. Execution in the Kingdom of Nouns,  http://steve-yegge.blogspot.com.au/2006/03/execution-in-kingdom-of-nouns.html

\bibitem{SOLID} SOLID (object-oriented design), https://en.wikipedia.org/wiki/SOLID\_(object-oriented\_design)
