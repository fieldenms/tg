...
public class WidgetSelector<T extends AbstractEntity<?>> implements IWidgetSelector<T> {
    ...
    public IAutocompleterConfig<T> asAutocompleter() {
        final Class<? extends AbstractEntity<?>> propType = StringUtils.isEmpty(propertyName) ? smBuilder.entityType
                : (Class<? extends AbstractEntity<?>>) PropertyTypeDeterminator.determinePropertyType(smBuilder.entityType, propertyName);
