...
/// The first call fixes the layout kind and returns a kind-locked continuation, so every breakpoint of a master shares one kind.
///
public interface ILayoutConfig<T extends AbstractEntity<?>> {
    ...
    /// Installs a grid layout for the given `device` and `orientation`.
