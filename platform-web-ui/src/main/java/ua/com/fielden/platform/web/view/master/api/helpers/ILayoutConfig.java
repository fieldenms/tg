...
public interface ILayoutConfig<T extends AbstractEntity<?>> {
    ...
    /// Installs a layout of any kind (flex, grid, …) for the given `device` and `orientation`.
    /// The kind is carried by `layout` (an [ILayoutConfiguration]), so this contract is independent of the layout kind.
    ///
    ILayoutConfigWithDimensionsAndDone<T> setLayoutFor(final Device device, final Optional<Orientation> orientation, final ILayoutConfiguration layout);

    /// Installs a flex layout for the given `device` and `orientation` from its layout string.
    ...
    ///
    default ILayoutConfigWithDimensionsAndDone<T> setLayoutFor(final Device device, final Optional<Orientation> orientation, final String flexString) {
