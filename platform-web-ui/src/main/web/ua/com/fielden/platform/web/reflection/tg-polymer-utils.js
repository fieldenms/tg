...
/**
 * Returns deepest possible DOM element under [x; y] point.
 * Use 'document' or some internal 'element.shadowRoot' as a starting point.
 * This method iterates through Shadow DOMs of web components to find deepest element.
 */
export const elementFromPoint = function (documentOrShadowRoot, x, y) {
    const element = documentOrShadowRoot.elementFromPoint(x, y); // find elementFromPoint using standard API
    if (!element) { // if there is no element under point, just return 'null' (consistent with documentOrShadowRoot.elementFromPoint)
        return null;
    } else if (element.shadowRoot) { // if element is web component with shadow root, need to traverse in its internal DOM further
        if (element.shadowRoot === documentOrShadowRoot) {
            ...
            return elementFromPoint(element.shadowRoot, x, y);
        }
    } else {
        return element; // return non-web-component (no .shadowRoot) element "as is"
    }
};

/**
 * Returns deepest possible DOM element's direct child node under [x; y] point, that satisfies 'nodeFilter'.
 * Use 'document' or some internal 'element.shadowRoot' as a starting point.
 * This method iterates through Shadow DOMs of web components to find deepest element, for which its direct children nodes will be checked.
 */
export const nodeFromPoint = function (documentOrShadowRoot, x, y, nodeFilter) {
    const element = elementFromPoint(documentOrShadowRoot, x, y);
    if (!element) { // if there is no element under point, just return 'null'
        return null;
    } 
    // see https://stackoverflow.com/questions/13597157/get-dom-text-node-from-point for more details
    return Array.from(element.childNodes)
        .filter(nodeFilter) // only consider nodes filtered by 'nodeFilter'
        .find(node => {
            const range = document.createRange(); // create Range
            range.selectNode(node); // and select 'node' to check
            return Array.from(range.getClientRects())
                .some(rect => x > rect.left && x < rect.right && y > rect.top && y < rect.bottom); // if there is some client rectangle, that contains node, then the node was found
        }) || null; // if there is no node, return 'null' (instead of 'undefined')
};