...
<script>
    Polymer({
        ...
        behaviors: [Polymer.IronResizableBehavior, Polymer.TgBehaviors.TgTooltipBehavior, Polymer.TgBehaviors.TgFocusRestorationBehavior],
        ...
        _pageSelectionChanged: function (event) {
            ...
            if (target === this.$.views) {
                const egi = this.querySelector(".custom-egi");
        ...
        _getVisibleFocusableElementIn: function (container) {
            return Array.from(container.querySelectorAll(FOCUSABLE_ELEMENTS_SELECTOR)).filter(element => !element.disabled && element.offsetParent !== null)[0];
        },
