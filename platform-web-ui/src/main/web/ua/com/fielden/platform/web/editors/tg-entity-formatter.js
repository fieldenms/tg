...
const states = {
    ...
    's2': (entity, template, idx, reflector, titles) => {
        if (template[idx] === 't') {
            ...
            titles[titles.length - 1].value = reflector.tg_toString(reflector.tg_convert(entity.get(name)), entity.type(), name);
...
export function composeDefaultEntityValue(entity) {
    if (entity.type().isCompositeEntity()) {
        const titles = [];
        createCompositeTitleWithoutTemplate(entity, new TgReflector(), titles);
        return titles;
    }
    return createSimpleTitle(entity);
}
...
function composeDefaultValueObject(entity, reflector, titles) {
    ...
    entityType.compositeKeyNames().forEach(keyName => {
        if (entity.get(keyName)) {
            titles.push({
                value: reflector.tg_toString(reflector.tg_convert(entity.get(keyName)), entity.type(), keyName),
...
function createSimpleTitle (entity) {
    return [{value: entity.toString()}]; // entity never empty
}