...
public class QueryModelCompositionTest extends BaseEntQueryCompositionTCase {
    ...
    public void test_query_model3() {
        ...
        //	.condition(null)
        //	.and()
        .beginExpr().prop("a").add().prop("b").endExpr().in().props(new String[] {}).groupBy().param("aaa").groupBy().prop("a").groupBy().beginExpr().param("11").add().prop("2").mult().val(100).endExpr().yield().param("a").as("a").yield().param("b").as("b").yield().count().days().between().model(null).and().beginExpr().beginExpr().beginExpr().beginExpr().param("1").endExpr().endExpr().endExpr().endExpr().as("myExp").yield().beginExpr().beginExpr().ifNull().prop("1").then().now().endExpr().endExpr().as("mySecondExp").yield().beginExpr().avgOf().beginExpr().prop("1").add().param("2").endExpr().add().val(3).endExpr().as("avg_from1+3").yield().avgOf().beginExpr().beginExpr().prop("1").add().prop("2").endExpr().endExpr().as("avg_of_1+2").yield().beginExpr().beginExpr().avgOf().beginExpr().beginExpr().beginExpr().beginExpr().count().days().between().prop("1").and().val(100).endExpr().endExpr().endExpr().endExpr().add().val(2).endExpr().div().val(100).endExpr().as("alias").yield().beginExpr().beginExpr().sumOf().prop("1").add().avgOf().prop("2").add().beginExpr().avgOf().beginExpr().beginExpr().beginExpr().beginExpr().val(2).endExpr().endExpr().endExpr().endExpr().endExpr().endExpr().add().avgOf().beginExpr().val(2).add().prop("p1").endExpr().endExpr().as("alias").yield().caseWhen().prop("1").eq().prop("2").and().val(1).ge().val(2).then().val(1).end().as("aaa").yield().round().prop("a").to(10).as("1").yield().param("aa").as("aaa").yield().countOf().prop("1").as("a").yield().yearOf().now().as("b")
        //	.yield().join("b").as("c")
