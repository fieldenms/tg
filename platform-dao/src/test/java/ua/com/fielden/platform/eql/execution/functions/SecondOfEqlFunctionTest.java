...
public class SecondOfEqlFunctionTest extends AbstractEqlExecutionTestCase {

    @Test
    public void evaluates_to_null_when_operand_is_null() {
        final var qry = select().yield().secondOf().val(null).as(RESULT).modelAsAggregate();
        assertNull(retrieveResult(qry));
    }

    @Test
    public void works_when_operand_is_value() {
        final var qry = select().yield().secondOf().val(date("2024-05-29 00:00:04")).as(RESULT).modelAsAggregate();
        assertEquals(valueOf(4), retrieveResult(qry));
    }

    @Test
    public void works_when_operand_is_property() {
        final var qry = select(TeNamedValuesVector.class).yield().secondOf().prop("dateAndTimeOf20010911084640").as(RESULT).modelAsAggregate();
    ...
    public void works_when_operand_is_function() {
        final var qry = select(TeNamedValuesVector.class).yield().
                secondOf().ifNull().prop("dateOfNullValue").then().val(date("2001-01-01 23:59:59")).
    ...
    public void works_when_operand_is_subquery() {
        final var qry = select().yield().
                secondOf().model(select(TeNamedValuesVector.class).yield().prop("dateAndTimeOf20010911084640").modelAsPrimitive()).
                as(RESULT).modelAsAggregate();
        assertEquals(valueOf(40), retrieveResult(qry));
    }
    ...
    public void works_when_the_function_result_is_used_within_other_query_expressions_and_conditions() {
        final var qry = select(TeNamedValuesVector.class).yield().caseWhen().val(40).eq().secondOf().prop("dateAndTimeOf20010911084640").then().val(1).end().as(RESULT).modelAsAggregate();
