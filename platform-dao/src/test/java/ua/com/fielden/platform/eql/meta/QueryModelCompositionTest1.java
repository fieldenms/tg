...
public class QueryModelCompositionTest1 extends BaseEntQueryCompositionTCase1 {
    protected final IWhere0<TgtVehicle> where_veh = select(VEHICLE).where();
    protected final IWhere0<TgtWorkOrder> where_wo = select(WORK_ORDER).as("wo").where();
    ...
    public void test_plain_quantified_test() {
        final EntityResultQueryModel<TgtVehicleModel> vehModels = select(MODEL).model();
        assertModelsEquals( //
    ...
    public void test_expressions1() {
        ...
        assertModelsEquals(//
        conditions(new ComparisonTest1(expression(expression(expression(expression(prop("wo.actCost.amount"), compound(_mult, param("costMultiplier"))), //
                compound(_add, prop("wo.estCost.amount")), compound(_div, param("costDivider"))), //
                compound(_add, prop("wo.yearlyCost.amount"))), //
                compound(_div, val(12))), _gt, val(1000))), //

        conditions(where_wo.beginExpr().beginExpr(). //
    ...
    public void test_query_sources() {
        final EntityResultQueryModel<TgtVehicle> qry = select(VEHICLE).as("v").join(TgtWorkOrder.class).as("wo").on().prop("v").eq().prop("wo.vehicle").leftJoin(TgtWorkOrder.class).as("wo2").on().prop("v").eq().prop("wo2.vehicle"). //
        ...
        others.add(new CompoundSource1(new TypeBasedSource1(DOMAIN_METADATA_ANALYSER.getPersistedEntityMetadata(WORK_ORDER), "wo"), JoinType.IJ, condition1));
        others.add(new CompoundSource1(new TypeBasedSource1(DOMAIN_METADATA_ANALYSER.getPersistedEntityMetadata(WORK_ORDER), "wo2"), JoinType.LJ, condition2));

        final EntQuery1 act = entResultQry(qry);
        final Sources1 exp = new Sources1(new TypeBasedSource1(DOMAIN_METADATA_ANALYSER.getPersistedEntityMetadata(VEHICLE), "v"), others);
    ...
    public void test_query_with_one_group() {
        final EntityResultQueryModel<TgtVehicleModel> qry = select(VEHICLE).groupBy().prop("model").yield().prop("model").modelAsEntity(MODEL);
        final EntQuery1 act = entResultQry(qry);
    ...
    @Test
    public void test2() {
        final EntityResultQueryModel<TgtVehicleModel> qry = select(VEHICLE).yield().prop("model").modelAsEntity(MODEL);
        final Yields1 exp = new Yields1();
        exp.addYield(new Yield1(prop("model"), ""));
    ...
    @Test
    public void test4() {
        final EntityResultQueryModel<TgtVehicleModel> qry = select(VEHICLE).as("v").yield().prop("v.model").modelAsEntity(MODEL);
        final Yields1 exp = new Yields1();
    ...
    public void test_validation_of_yielded_tree_with_broken_hierarchy() {
        ...
        final AggregatedResultQueryModel sourceQry = select(TgtFuelUsage.class). //
    ...
    public void test_query_sources1() {
        final EntityResultQueryModel<TgtVehicle> qry = select(VEHICLE).as("v").join(WORK_ORDER).as("wo").on().prop("v").eq().prop("vehicle").model();
        ...
        others.add(new CompoundSource1(new TypeBasedSource1(DOMAIN_METADATA_ANALYSER.getPersistedEntityMetadata(WORK_ORDER), "wo"), JoinType.IJ, condition));
    ...
    public void test_query_sources1a() {
        final EntityResultQueryModel<TgtVehicle> qry = select(VEHICLE).as("v").join(WORK_ORDER).as("wo").on().prop("v").eq().prop("wo.vehicle").where().val(1).isNotNull().model();
    ...
    public void test_query_sources_with_explicit_join_and_without_aliases() {
        final EntityResultQueryModel<TgtVehicle> qry = select(VEHICLE).as("v").join(WORK_ORDER).on().prop("v").eq().prop("vehicle").model();

        final Conditions1 condition = conditions(new ComparisonTest1(prop("v"), ComparisonOperator.EQ, prop("vehicle")));

        final List<CompoundSource1> others = new ArrayList<CompoundSource1>();
        others.add(new CompoundSource1(new TypeBasedSource1(DOMAIN_METADATA_ANALYSER.getPersistedEntityMetadata(WORK_ORDER), null), JoinType.IJ, condition));
    ...
    public void test_query_sources2() {
        final EntityResultQueryModel<TgtVehicle> qry = select(VEHICLE).as("v").join(WORK_ORDER).as("wo").on().prop("v").eq().prop("wo.vehicle").leftJoin(WORK_ORDER).as("wo2").on().prop("v").eq().prop("wo2.vehicle").model();
    ...
    public void test_query_with_derived_sources() {
        final EntityResultQueryModel<TgtVehicle> sourceQry = select(VEHICLE).as("v").where().prop("v.model").isNotNull().model();
        final EntityResultQueryModel<TgtVehicle> qry = select(sourceQry).as("v").where().prop("v.model").isNotNull().model();
    ...
    public void test_simple_query_model_13_() {
        final EntityResultQueryModel<TgtVehicle> qry = select(VEHICLE).as("v").model();
        final List<CompoundSource1> others = new ArrayList<CompoundSource1>();
        final Sources1 exp = new Sources1(new TypeBasedSource1(DOMAIN_METADATA_ANALYSER.getPersistedEntityMetadata(VEHICLE), "v"), others);
        assertEquals("models are different", exp, entResultQry(qry).getSources());
    }
    ...
    public void test_simple_query_model_14() {
        final EntityResultQueryModel<TgtVehicle> qry = select(VEHICLE).model();
        final List<CompoundSource1> others = new ArrayList<CompoundSource1>();
        final Sources1 exp = new Sources1(new TypeBasedSource1(DOMAIN_METADATA_ANALYSER.getPersistedEntityMetadata(VEHICLE), null), others);
        assertEquals("models are different", exp, entResultQry(qry).getSources());
    }
    ...
    public void test_query_model1() {
        final AggregatedResultQueryModel a = select(WORK_ORDER).as("wo").where().beginExpr().beginExpr().beginExpr().beginExpr().prop("wo.actCost.amount").mult().param("costMultiplier").endExpr().add().prop("wo.estCost.amount").div().param("costDivider").endExpr().add().prop("wo.yearlyCost.amount").endExpr().div().val(12).endExpr().gt().val(1000).and().begin().beginExpr().param("param1").mult().beginExpr().param("v").add().prop("wo.vehicle.initPrice").endExpr().endExpr().eq().prop("wo.insuranceAmount").and().val("a").isNull().and().prop("wo.insuranceAmount").isNotNull().end().modelAsAggregate();
        entResultQry(a);//.getPropNames();

        select(WORK_ORDER).where().upperCase().beginExpr().prop("bbb").add().prop("aaa").endExpr().like().upperCase().val("AaA").and().prop("SomeDate").gt().now().and().yearOf().prop("LastChange").eq().round().beginExpr().prop("prop1").add().prop("prop2").endExpr().to(1).and().param("a").eq().ifNull().prop("interProp").then().val(1).and().prop("aaa").eq().count().days().between().beginExpr().beginExpr().prop("Start1").add().prop("Start2").endExpr().endExpr().and().ifNull().prop("End").then().now().and().prop("haha").in().props("p1", "p2", "p3").and().param("AAA").in().values(1, 2, 3).modelAsAggregate();
    ...
    public void test_query_model3() {
        final Object a = select(WORK_ORDER).as("a").join(AbstractEntity.class).as("b").on().prop("a").eq().prop("v").and().begin().begin().beginExpr().prop("a").add().param("v").endExpr().eq().prop("c").end().end().and().expr(null).eq().expr(null).and().prop("a").eq().beginExpr().prop("1").sub().prop("2").endExpr().and().anyOfModels(null, null).eq().beginExpr().prop("a").endExpr().and().beginExpr().beginExpr().beginExpr().beginExpr().param("1").add().param("2").endExpr().add().beginExpr().param("1").div().prop("2").endExpr().endExpr().endExpr().endExpr().eq().val(false).and().notExists(null).where().count().days().between().prop("1").and().prop("2").eq().beginExpr().count().days().between().model(null).and().model(null).endExpr().and().prop("1").ne().param("a").and().beginExpr().prop("1").add().beginExpr().prop("1").add().prop("2").add().beginExpr().beginExpr().param("1").div().param("2").endExpr().div().beginExpr().param("3").endExpr().endExpr().endExpr().sub().prop("2").endExpr().eq().prop("a").and().beginExpr().prop("2").endExpr().eq().beginExpr().prop("1").sub().prop("2").endExpr().and().begin().prop("asa").isNull().end().and().beginExpr().prop("dsd").endExpr().eq().param("ss").and().prop("a").like().beginExpr().val(1).add().val(3).endExpr().and().begin().beginExpr().prop("1").sub().prop("2").endExpr().eq().prop("3").and().begin().beginExpr().prop("1").sub().val(2).endExpr().isNotNull().and().begin().exists(null).and().beginExpr().prop("1").div().val(2).mult().val(3).endExpr().eq().beginExpr().prop("1").add().prop("2").endExpr().end().end().end().and().count().days().between().beginExpr().beginExpr().beginExpr().val(1).sub().val(2).endExpr().endExpr().endExpr().and().beginExpr().beginExpr().beginExpr().param("1").div().param("2").endExpr().endExpr().endExpr().eq().count().days().between().prop("1").and().now().and().beginExpr().param("1").sub().upperCase().prop("1").endExpr().gt().all(null).and().val(1).isNotNull().and().beginExpr().prop("1").add().prop("2").endExpr().in().model(null).and().ifNull().prop("1").then().upperCase().val("AaAv").eq().beginExpr().ifNull().beginExpr().prop("1").add().prop("2").endExpr().then().now().sub().val(1).endExpr().and().anyOfModels(null, null).eq().prop("a").and().begin().begin().prop("a").eq().beginExpr().beginExpr().beginExpr().beginExpr().now().endExpr().endExpr().endExpr().endExpr().and().begin().beginExpr().beginExpr().beginExpr().beginExpr().now().endExpr().endExpr().endExpr().endExpr().eq().beginExpr().beginExpr().beginExpr().beginExpr().param("a").endExpr().endExpr().endExpr().add().beginExpr().beginExpr().beginExpr().prop("a").endExpr().endExpr().endExpr().endExpr().end().end().end().or().caseWhen().prop("1").eq().prop("2").and().val(1).ge().val(2).then().val(1).end().ge().param("bb").and()
        ...
        //	.condition(null)
        //	.and()
        .beginExpr().prop("a").add().prop("b").endExpr().in().props().groupBy().param("aaa").groupBy().prop("a").groupBy().beginExpr().param("11").add().prop("2").mult().val(100).endExpr().yield().param("a").as("a").yield().param("b").as("b").yield().count().days().between().model(null).and().beginExpr().beginExpr().beginExpr().beginExpr().param("1").endExpr().endExpr().endExpr().endExpr().as("myExp").yield().beginExpr().beginExpr().ifNull().prop("1").then().now().endExpr().endExpr().as("mySecondExp").yield().beginExpr().avgOf().beginExpr().prop("1").add().param("2").endExpr().add().val(3).endExpr().as("avg_from1+3").yield().avgOf().beginExpr().beginExpr().prop("1").add().prop("2").endExpr().endExpr().as("avg_of_1+2").yield().beginExpr().beginExpr().avgOf().beginExpr().beginExpr().beginExpr().beginExpr().count().days().between().prop("1").and().val(100).endExpr().endExpr().endExpr().endExpr().add().val(2).endExpr().div().val(100).endExpr().as("alias").yield().beginExpr().beginExpr().sumOf().prop("1").add().avgOf().prop("2").add().beginExpr().avgOf().beginExpr().beginExpr().beginExpr().beginExpr().val(2).endExpr().endExpr().endExpr().endExpr().endExpr().endExpr().add().avgOf().beginExpr().val(2).add().prop("p1").endExpr().endExpr().as("alias").yield().caseWhen().prop("1").eq().prop("2").and().val(1).ge().val(2).then().val(1).end().as("aaa").yield().round().prop("a").to(10).as("1").yield().param("aa").as("aaa").yield().countOf().prop("1").as("a").yield().yearOf().now().as("b")
        //	.yield().join("b").as("c")
