...
public class QueryShortcutsTest extends BaseEntQueryTCase {
    ...
    public void test_model_with_expression() {
        select(VEHICLE).where().beginExpr().val(100).mult().model(select(FUEL_USAGE).yield().sumOf().prop("qty").modelAsPrimitive()).endExpr().ge().val(1000).model();
    }
    ...
    @Test
    public void user_data_filter_condition_is_correctly_attached_to_query_with_main_source_being_filtered_entity_type() {
