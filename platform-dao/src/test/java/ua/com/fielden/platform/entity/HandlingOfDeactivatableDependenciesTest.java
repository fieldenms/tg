...
public class HandlingOfDeactivatableDependenciesTest extends AbstractDomainDrivenTestCase {
    ...
    public void active_person_with_active_authoriser_and_originator_deactivatable_dependencies_and_one_tangenital_dependeny_should_have_one_ref_count() {
        final TgPerson person = ao(TgPerson.class).findByKey("P1");
    ...
    public void active_person_with_acitve_authoriser_and_inactive_originator_deactivatable_dependencies_should_have_zero_ref_count() {
        final TgPerson person = ao(TgPerson.class).findByKey("P2");
    ...
    public void deactivating_authoriser_that_references_active_person_should_not_change_its_ref_count() {
        final TgPerson person = ao(TgPerson.class).findByKey("P1");
        assertEquals(Integer.valueOf(1), person.getRefCount());
        final TgAuthoriser auth = ao(TgAuthoriser.class).findByKey(person);

        save(auth.setActive(false));

        assertEquals(Integer.valueOf(1), ao(TgPerson.class).findByKey("P1").getRefCount());
    }

    @Test
    public void there_suppose_to_be_two_deactivatable_dependencies_for_person_P1() {
        final TgPerson p1 = ao(TgPerson.class).findByKey("P1");
        ...
        assertTrue(deps.contains(ao(TgAuthoriser.class).findByKey(p1)));
        assertTrue(deps.contains(ao(TgOriginator.class).findByKey(p1)));

        final TgPerson p3 = ao(TgPerson.class).findByKey("P3");
        assertFalse(deps.contains(ao(TgOriginator.class).findByKey(p3)));
    }
    ...
    public void there_suppose_to_be_one_deactivatable_dependencies_for_person_P2() {
        ...
        assertTrue(deps.contains(ao(TgAuthoriser.class).findByKey(person)));
    }
    ...
    public void deactivating_originator_that_references_two_active_persons_should_not_change_ref_count_for_one_where_it_is_dependable_and_should_change_for_the_other() {
        ...
        final TgOriginator orig = ao(TgOriginator.class).findByKey(p3);

        save(orig.setActive(false));

        assertEquals("P1 was referenced by just deactivated originator as an assistant, and should have its refCount decremented.", Integer.valueOf(0), ao(TgPerson.class).findByKey("P1").getRefCount());
        assertEquals("P3 was referenced by just deactivated originator as one of key members, and should not have its refCount effected.",Integer.valueOf(0), ao(TgPerson.class).findByKey("P3").getRefCount());
    }
    ...
    public void activating_originator_that_references_two_active_persons_should_not_change_ref_count_for_one_where_it_is_dependable_and_should_change_for_the_other() {
        final TgPerson p2 = ao(TgPerson.class).findByKey("P2");
        final TgOriginator orig = ao(TgOriginator.class).findByKey(p2);

        save(orig.setActive(true));

        assertEquals("P1 is referenced by just activated originator as an assistant, and should have its refCount incremented.", Integer.valueOf(2), ao(TgPerson.class).findByKey("P1").getRefCount());
        assertEquals("P2 is referenced by just activated originator as one of key members, and should not have its refCount effected.",Integer.valueOf(0), ao(TgPerson.class).findByKey("P2").getRefCount());
    }
    ...
    public void deactivation_of_person_p3_with_originators_assistant_as_p1_should_decrement_p1s_ref_count() {
        ...
        final TgPerson p3 = save(ao(TgPerson.class).findByKey("P3").setActive(false));
        assertFalse(ao(TgAuthoriser.class).findByKey(p3).isActive());
        assertFalse(ao(TgOriginator.class).findByKey(p3).isActive());
        assertEquals(Integer.valueOf(0), ao(TgPerson.class).findByKey("P1").getRefCount());
    }
    ...
    public void activation_of_inactive_authoriser_should_increase_ref_count_for_referenced_category_and_decrease_it_upon_automatic_deactivation() {
        final TgPerson p3 = ao(TgPerson.class).findByKey("P3");
        final TgAuthoriser auth = ao(TgAuthoriser.class).findByKey(p3);
        assertEquals(Integer.valueOf(0), ao(TgCategory.class).findByKey("CAT1").getRefCount());

        save(auth.setActive(true));
        assertEquals(Integer.valueOf(1), ao(TgCategory.class).findByKey("CAT1").getRefCount());
