...
public class MenuSaveActionTest extends AbstractDaoTestCase {
    ...
    @Test
    public void only_base_user_can_save_module_menu_changes() {
        ...
        final List<WebMenuItemInvisibility> menuItems = co(WebMenuItemInvisibility.class).
                getAllEntities(from(select(WebMenuItemInvisibility.class).where().prop("owner").eq().val(co(User.class).findByKey("USER_2")).model()).
