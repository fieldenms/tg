...
public class PropertyPersistenceInfoTest extends BaseEntQueryTCase {
	...
	final PropertyMetadata bogieLocationProp = new PropertyMetadata.Builder("location", TgBogieLocation.class, true). //
		...
		type(PropertyCategory.UNION_ENTITY). //
		build();
	final SortedSet<PropertyMetadata> expected = new TreeSet<PropertyMetadata>();
	expected.add(ppi("location.wagonSlot", WAGON_SLOT, true, hibType("long"), "LOCATION_WAGONSLOT", PropertyCategory.UNION_DETAILS));
	expected.add(ppi("location.workshop", WORKSHOP, true, hibType("long"), "LOCATION_WORKSHOP", PropertyCategory.UNION_DETAILS));
