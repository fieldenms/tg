 ...
 * <p>
 * First, the collection <tt>items</tt> is a true many-to-many association, with collections on both sides. There are no additional columns in the underlying many-to-many join
 * table.
 * <p>
 * Second, the collection <tt>categorizedItems</tt> is a one-to-many association to an entity class <tt>CategorizedItem</tt> that represents the link. The <tt>Item</tt> class
 * has the same collection mapped, to make it bidirectional. This intermediate class represents additional columns on the many-to-many join table, such as the user who added the
 * item to the category, and the date of the addition.
 * <p>
 * Third, the collection <tt>categorizedItemComponents</tt> is a collection of value typed elements, of value type <tt>CategorizedItemComponent</tt>. This simplifies
 * management of the link (no intermediate entity class) but allows only unidirectional navigation. The <tt>Item</tt> class does not know anything about this collection or the
 * components - no shared references.
 * <p>
 * Finally, the map <tt>itemsAndUser</tt> represents the many-to-many association with a ternary relationship using a hash map. This map has item objects as keys, and user
 * objects as values. The underlying many-to-many join table has three columns, <tt>CATEGORY_ID</tt>, <tt>ITEM_ID</tt>, and <tt>ADDED_BY_USER_ID</tt>. This strategy allows
 * you to map an additional column (the user foreign key) of a many-to-many join table without writing an intermediate entity or component class.
 * 
 * @see Item
 * @see CategorizedItem
...
public class Category implements Serializable, Comparable {
     ...
     * Full constructor
     */
    public Category(String name, List<Category> childCategories, Category parentCategory, List<Item> items, Set<CategorizedItem> categorizedItems, Set<CategorizedItemComponent> categorizedItemComponents, Map<Item, User> itemsAndUser) {
	this.name = name;
