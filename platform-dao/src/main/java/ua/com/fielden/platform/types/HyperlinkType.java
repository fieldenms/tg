...
public class HyperlinkType implements UserType, IHyperlinkType {
    ...
    public Object instantiate(final Object argument, final EntityFactory factory) {
        ...
        try {
            return new Hyperlink.Persisted((String) argument);
        } catch (final Exception e) {
            throw new UserTypeException("Could not instantiate instance of [%s] with value [%s] due to: %s.".formatted(Hyperlink.class.getName(), argument, e.getMessage()), e);
        }
    }
    ...
    public Object nullSafeGet(final ResultSet resultSet, final String[] names, final SharedSessionContractImplementor session, final Object owner) throws SQLException {
        ...
        if (!resultSet.wasNull()) {
            try {
                result = new Hyperlink.Persisted(name);
            } catch (final Exception e) {
                throw new UserTypeException("Hyperlink for value [%s] could not be instantiated.".formatted(name), e);
            }
        }
        return result;
    }
