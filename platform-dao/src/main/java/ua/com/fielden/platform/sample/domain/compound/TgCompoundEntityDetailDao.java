...
public class TgCompoundEntityDetailDao extends CommonEntityDao<TgCompoundEntityDetail> implements ITgCompoundEntityDetail {
    ...
    public TgCompoundEntityDetail save(final TgCompoundEntityDetail entity) {
        ...
        if (entity.getId() != null && (entity.getDesc() != null && entity.getDesc()
                .endsWith(" detail_CHANGED") /* 1TEST and NEWXX */ || "1TEST detail".equals(
                entity.getDesc()))) { // if we add / remove '_CHANGED' part for detail entity description ...
            final TgCompoundEntity masterEntity = co$(TgCompoundEntity.class).findById(entity.getId(),
                                                                                       co$(TgCompoundEntity.class).getFetchProvider()
                                                                                               .fetchModel()); // ... then fetch master entity ...
            masterEntity.setDesc(format("%s (%s)", masterEntity.getKey(),
                                        entity.getDesc())); // ... and change its description accordingly ...
