...
//		final Integer count = count(new QueryExecutionModel<T>(createQueryByKey(entity.getKey()), null, null, null));
//		if (count > 0) {
...
//		// let's also make sure that duplicate entities are not allowed
//		final UnorderedQueryModel model = select(createQueryByKey(entity.getKey())).yield().prop("id").modelAsPrimitive();
//		final EntityFetcher<EntityAggregates> fetcher = new EntityFetcher<EntityAggregates>();
...
//	final List<T> result = new EntityFetcher<T>().list(getSession(), mappingExtractor, entityFactory, queryModel.enhanceWith(filter, getUsername()), pageNumber, pageCapacity, fetchModel);
