...
public class EqlQueryTransformer {
    public static <E extends AbstractEntity<?>> TransformationResult<ResultQuery3> transform(final QueryExecutionContext executionContext, final QueryProcessingModel<E, ?> qem, final DbVersion dbVersion, final IFilter filter, final String username) {
        final EntQueryGenerator gen = new EntQueryGenerator(dbVersion, filter, username, executionContext.dates(), qem.getParamValues(), executionContext.getDomainMetadata().eqlDomainMetadata);
        final TransformationContext context = new TransformationContext(executionContext.getDomainMetadata().eqlDomainMetadata);
        final ResultQuery2 query2 = gen.generateAsResultQuery(qem.queryModel, qem.orderModel, qem.fetchModel).transform(context);
        final PathsToTreeTransformator p2tt = new PathsToTreeTransformator(executionContext.getDomainMetadata().eqlDomainMetadata, gen);
        final Map<String, List<ChildGroup>> childGroupsMap = p2tt.groupChildren(query2.collectProps());
        return query2.transform(new ua.com.fielden.platform.eql.stage2.TransformationContext(new TablesAndSourceChildren(executionContext.getDomainMetadata().eqlDomainMetadata.getTables(), childGroupsMap)));
    }
}