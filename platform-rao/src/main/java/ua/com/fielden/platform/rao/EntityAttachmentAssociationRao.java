...
public class EntityAttachmentAssociationRao extends CommonEntityRao<EntityAttachmentAssociation> implements IEntityAttachmentAssociationController {
	...
	return new SinglePage<EntityAttachmentAssociation>(getAllEntities(from(q).with(new fetchAll<EntityAttachmentAssociation>(EntityAttachmentAssociation.class)).with(orderBy).build()));
    }
